...
type drawGraph(sv:svgfilemaker, q:Quadrant, x1,y1,x2,y2, marginX,marginY) =
    ...
    /// 描画範囲(グラフ内ローカル座標系)
    member _.X1 with get() = X1_
    /// 描画範囲(グラフ内ローカル座標系)
    member _.Y1 with get() = Y1_
    /// 描画範囲(グラフ内ローカル座標系)
    member _.X2 with get() = X2_
    /// 描画範囲(グラフ内ローカル座標系)
    ...
    member _.arrow ((x1:double,y1:double),(x2:double,y2:double),col:color.stroke) =
    ...
    /// 点を描画
    member _.point (x:double,y:double) (size:double,fcol:color.fill,scol:color.stroke) =
    ...
    member this.polygon (plist:(double*double)list) (scol:color.stroke) =
        let xlist = plist |> List.map (fun (x,_) -> transX x)
        let ylist = plist |> List.map (fun (_,y) -> transY y)
...
module graph =
    ...
    /// 指定したレイアウトでグラフ作成
    let make (environment:Aqualis) (L:Layout) (outputdir:string) filename (nX:int,nY:int) code =
