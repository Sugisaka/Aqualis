...
namespace Aqualis
    ...
    type omp =
        ...
        ///<summary>ループを並列化</summary>
        static member reduction (var:double0,ope:string) = fun code ->
            match (GenerationScope.currentProgram()).language with
            |Fortran ->
                (GenerationScope.requireContext()).IsOpenMpUsed <- true
                (GenerationScope.currentProgram()).mlist.add "omp_lib"
        ...
        ///<summary>ループを並列化</summary>
        static member reduction (var:complex0,ope:string) = fun code ->
            match (GenerationScope.currentProgram()).language with
            |Fortran ->
                (GenerationScope.requireContext()).IsOpenMpUsed <- true
                (GenerationScope.currentProgram()).mlist.add "omp_lib"
        ...
        ///<summary>ループを並列化</summary>
        static member reduction_th (th:int,var:double0,ope:string) = fun code ->
            match (GenerationScope.currentProgram()).language with
            |Fortran ->
                (GenerationScope.requireContext()).IsOpenMpUsed <- true
                (GenerationScope.currentProgram()).mlist.add "omp_lib"
            ...
            |_ ->
                ()                
        ///<summary>ループを並列化</summary>
        static member reduction_th (th:int,var:complex0,ope:string) = fun code ->
            match (GenerationScope.currentProgram()).language with
            |Fortran ->
                (GenerationScope.requireContext()).IsOpenMpUsed <- true
                (GenerationScope.currentProgram()).mlist.add "omp_lib"
        ...
        static member thread_num with get() =
            match (GenerationScope.currentProgram()).language with
            |Fortran |C99 ->
                int0(Var(It 4,"omp_get_thread_num()",NaN))
            |_ ->
        ...
        static member max_threads with get() =
            match (GenerationScope.currentProgram()).language with
            |Fortran |C99 ->
                int0(Var(It 4,"omp_get_max_threads()",NaN))
            |_ ->
