...
namespace Aqualis
    ...
    and num0(x:expr, ?context:GenerationContext) =
        ...
        static member (===) (x:num0,y:num0) = num0(Var(Nt,x.code+" = "+y.code,NaN)) //expr.equiv x.Expr y.Expr ((GenerationScope.currentProgram()))
        static member (===) (x:num0,y:int) = x === num0(Int y)
        static member (===) (x:num0,y:double) = x === num0(Dbl y)

        ///<summary>等号揃等式(TeX、HTMLのみ)</summary>
        static member (=|=) (x:num0,y:num0) = num0(Var(Nt,x.code+" =& "+y.code,NaN)) //expr.equivAlign x.Expr y.Expr ((GenerationScope.currentProgram()))
        static member (=|=) (x:num0,y:int) = x =|= num0(Int y)
        static member (=|=) (x:num0,y:double) = x =|= num0(Dbl y)
