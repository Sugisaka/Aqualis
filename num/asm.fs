...
namespace Aqualis
    ...
    type asm =
        ...
        static member iSum (n1:int, n2:int, f:int0->int0) = int0(Sum(It 4, Int n1, Int n2, fun (x:expr) -> (f(int0 x)).Expr))
        static member dSum (n1:int0, n2:int0, f:int0->double0) = double0(Sum(Dt, n1.Expr, n2.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member dSum (n1:int, n2:int0, f:int0->double0) = double0(Sum(Dt, Int n1, n2.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member dSum (n1:int0, n2:int, f:int0->double0) = double0(Sum(Dt, n1.Expr, Int n2, fun (x:expr) -> (f(int0 x)).Expr))
        static member dSum (n1:int, n2:int, f:int0->double0) = double0(Sum(Dt, Int n1, Int n2, fun (x:expr) -> (f(int0 x)).Expr))
        static member zSum (n1:int0, n2:int0, f:int0->complex0) = complex0(Sum(Zt, n1.Expr, n2.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member zSum (n1:int, n2:int0, f:int0->complex0) = complex0(Sum(Zt, Int n1, n2.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member zSum (n1:int0, n2:int, f:int0->complex0) = complex0(Sum(Zt, n1.Expr, Int n2, fun (x:expr) -> (f(int0 x)).Expr))
        static member zSum (n1:int, n2:int, f:int0->complex0) = complex0(Sum(Zt, Int n1, Int n2, fun (x:expr) -> (f(int0 x)).Expr))
        static member iLet (x:int0, f:int0->int0) = int0(Let(It 4, x.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member iLet (x:int, f:int0->int0) = asm.iLet (I x, f)
        static member dLet (x:int0, f:int0->double0) = double0(Let(Dt, x.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member dLet (x:double0, f:double0->double0) = double0(Let(Dt, x.Expr, fun (x:expr) -> (f(double0 x)).Expr))
        static member dLet (x:int, f:int0->double0) = asm.dLet (I x, f)
        static member dLet (x:double, f:double0->double0) = asm.dLet (D x, f)
        static member zLet (x:int, f:int0->complex0) = asm.zLet (I x, f)
        static member zLet (x:int0, f:int0->complex0) = complex0(Let(Zt, x.Expr, fun (x:expr) -> (f(int0 x)).Expr))
        static member zLet (x:double, f:double0->complex0) = asm.zLet (D x, f)
        static member zLet (x:double0, f:double0->complex0) = complex0(Let(Zt, x.Expr, fun (x:expr) -> (f(double0 x)).Expr))
        static member zLet (x:complex0, f:complex0->complex0) = complex0(Let(Zt, x.Expr, fun (x:expr) -> (f(complex0 x)).Expr))
        static member zLet (x:double*double, f:complex0->complex0) = asm.zLet (Z x, f)
