...
namespace Aqualis
    ...
    module geometry = 
        ...
        type point2_1(name,size1) =
            inherit base1(Structure(point2.sname),size1,name)
            //変数宣言を行う場合
            new(name,size1) =
                str.reg(point2.sname,name,size1)
                point2_1(name,A1(size1))
            member this.Item with get(i:num0) = point2(this.Idx1(i))
            member this.Item with get(i:int ) = point2(this.Idx1(i))
        ...
        type point3_1(sname,name,size1) =
            inherit base1(Structure(point3.sname),size1,name)
            new(name,size1) =
                str.reg(point3.sname,name,size1)
                point3_1(point3.sname,name,A1(size1))
            member this.Item with get(i:num0) = point3(this.Idx1(i))
            member this.Item with get(i:int ) = point3(this.Idx1(i))
            
        //#endregion
        
        //#region 5次元ベクトル    
        type Point5(x1:num0,x2:num0,x3:num0,x4:num0,x5:num0) =
            member public this.W1 with get() = x1
            member public this.W2 with get() = x2
            member public this.W3 with get() = x3
            member public this.W4 with get() = x4
            member public this.W5 with get() = x5
            static member (+) (a:Point5,b:Point5) =
              new Point5(a.W1+b.W1, a.W2+b.W2, a.W3+b.W3, a.W4+b.W4, a.W5+b.W5)
            
        type point5(name) =
            static member sname = "point5"
            new(name,c) =
                str.reg(point5.sname,name,c)
                point5(name)
            member public __.w1 = str.d0(point5.sname, name, "w1")
            member public __.w2 = str.d0(point5.sname, name, "w2")
            member public __.w3 = str.d0(point5.sname, name, "w3")
            member public __.w4 = str.d0(point5.sname, name, "w4")
            member public __.w5 = str.d0(point5.sname, name, "w5")
            member public this.abs with get() = asm.sqrt(this.w1*this.w1+this.w2*this.w2+this.w3*this.w3+this.w4*this.w4+this.w5*this.w5) 
            member public this.normalize() =
              ch.d <| fun norm ->
                norm <== this.abs
                this.w1 <== this.w1/norm
                this.w2 <== this.w2/norm
                this.w3 <== this.w3/norm
                this.w4 <== this.w4/norm
                this.w5 <== this.w5/norm
            static member (+) (a:point5,b:point5) = 
              new Point5(a.w1+b.w1, a.w2+b.w2, a.w3+b.w3, a.w4+b.w4, a.w5+b.w5)
            static member (-) (a:point5,b:point5) = 
              new Point5(a.w1-b.w1, a.w2-b.w2, a.w3-b.w3, a.w4-b.w4, a.w5-b.w5)
            static member (*) (a:num0,b:point5) = 
              new Point5(a*b.w1, a*b.w2, a*b.w3, a*b.w4, a*b.w5)
            static member (*) (a:point5,b:point5) = 
              a.w1 * b.w1 + a.w2 * b.w2 + a.w3 * b.w3 + a.w4 * b.w4 + a.w5 * b.w5
            static member (<==) (a:point5,b:Point5) = 
              a.w1 <== b.W1
              a.w2 <== b.W2
              a.w3 <== b.W3
              ...
              a.w5 <== b.W5
            static member str_mem(psname, vname, name) =
                str.addmember(psname,(Structure(point5.sname),A0,name))
                point5(structure.mem(vname,name))
          
        type point5_1(name,size1) =
            inherit base1(Structure(point5.sname),size1,name)
            new(name,size1) =
                str.reg(point5.sname,name,size1)
                point5_1(name,A1(size1))
            member this.Item with get(i:num0) = point5(this.Idx1(i))
            member this.Item with get(i:int ) = point5(this.Idx1(i))
            static member str_mem(sname, vname, name, size1) =
                str.addmember(sname,(Structure(sname),A1(size1),name))
                point5_1(structure.mem(vname,name),size1)
        ...
        let circle(x:num0,y:num0,center_x:num0,center_y:num0,radius:num0,d:num0,f:num0):num0 = 
            let rad(y:num0,x:num0,t:num0) =
                ...
                t
            codestr.section ("中心("+x.name+","+y.name+")、サイズ"+d.name+"の正方形領域に、"+"中心("+center_x.name+","+center_y.name+")、半径"+radius.name+"の円が占める割合を計算→結果は"+f.name) <| fun () ->
                ch.ii <| fun (Nx,Ny) ->
