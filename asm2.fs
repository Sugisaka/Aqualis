            ...
            static member random (code:(((num1->unit)->unit)*(num0->unit)->unit)) =
                ...
                match p.lang with
                  |F ->
                    ch.i <| fun seedsize ->
                    ch.i01 <| fun seed ->
                        p.codewrite("call random_seed(size="+seedsize.name+")")
                        seed.allocate(seedsize)
                        iter.num seedsize <| fun i ->
                            p.codewrite("call system_clock(count="+seed.name+"("+i.name+"))")
                            p.codewrite("call random_seed(put="+seed.name+"(:))")
                        let setseed code =
                            code seed
                            p.codewrite("call random_seed(put="+seed.name+"(:))")
                        let getrand (x:num0) =
                  ...
                  |T ->
                    ch.i1 _1 <| fun seed ->
                        let setseed code =
                            code seed
                            p.codewrite("random_seed="+seed.name+"[0])")
                        let getrand (x:num0) =
                            match x with
                              |Var(Dt,n,_) -> 
                ...
                match p.lang with
                  |F ->
                    ...
                    ch.i01 <| fun seed ->
                        ...
                        seed.allocate(seedsize)
                        iter.num seedsize <| fun i ->
                            p.codewrite(seed.name+"(:) = "+seed_.ToString())
                            p.codewrite("call random_seed(put="+seed.name+"(:))")
                        let setseed code =
                            code seed
                            p.codewrite("call random_seed(put="+seed.name+"(:))")
