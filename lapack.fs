(*
Copyright (c) 2022 Jun-ichiro Sugisaka

This software is released under the MIT License.
http://opensource.org/licenses/mit-license.php
*)
namespace Aqualis

    open Aqualis_base
    
    module lapack = 

        type La() =
            
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="x">a×b</param>
            /// <param name="a">a</param>
            /// <param name="b">b</param>
            static member matmul (x:complex1,a:complex2,b:complex1) =
                x.clear()
                iter.num a.size1 <| fun i ->
                    iter.num a.size2 <| fun j ->
                        x[i] <== x[i] + a[i,j] * b[j]
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="x">a×b</param>
            /// <param name="a">a</param>
            /// <param name="b">b</param>
            static member matmul (x:complex1,a:float2,b:complex1) =
                x.clear()
                iter.num a.size1 <| fun i ->
                    iter.num a.size2 <| fun j ->
                        x[i] <== x[i] + a[i,j] * b[j]
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="x">a×b</param>
            /// <param name="a">a</param>
            /// <param name="b">b</param>
            static member matmul (x:complex1,a:complex2,b:float1) =
                x.clear()
                iter.num a.size1 <| fun i ->
                    iter.num a.size2 <| fun j ->
                        x[i] <== x[i] + a[i,j] * b[j]
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="x">a×b</param>
            /// <param name="a">a</param>
            /// <param name="b">b</param>
            static member matmul (x:complex1,a:float2,b:float1) =
                x.clear()
                iter.num a.size1 <| fun i ->
                    iter.num a.size2 <| fun j ->
                        x[i] <== x[i] + a[i,j] * b[j]
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="x">a×b</param>
            /// <param name="a">a</param>
            /// <param name="b">b</param>
            static member matmul (x:float1,a:float2,b:float1) =
                x.clear()
                iter.num a.size1 <| fun i ->
                    iter.num a.size2 <| fun j ->
                        x[i] <== x[i] + a[i,j] * b[j]
                
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="a">行列</param>
            /// <param name="b">ベクトル</param>
            static member matmul (a:float2,b:float1) = fun code ->
                ch.d1 a.size1 <| fun x ->
                    La.matmul (x,a,b)
                    code x
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="a">行列</param>
            /// <param name="b">ベクトル</param>
            static member matmul (a:complex2,b:float1) = fun code ->
                ch.z1 a.size1 <| fun x ->
                    La.matmul (x,a,b)
                    code x
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="a">行列</param>
            /// <param name="b">ベクトル</param>
            static member matmul (a:float2,b:complex1) = fun code ->
                ch.z1 a.size1 <| fun x ->
                    La.matmul (x,a,b)
                    code x
            /// <summary>
            /// 行列×ベクトルの計算
            /// </summary>
            /// <param name="a">行列</param>
            /// <param name="b">ベクトル</param>
            static member matmul (a:complex2,b:complex1) = fun code ->
                ch.z1 a.size1 <| fun x ->
                    La.matmul (x,a,b)
                    code x
                    
            static member matmul (u:float2,a:float2,b:float2) =
                u.clear()
                iter.num a.size1 <| fun i ->
                    iter.num b.size2 <| fun j ->
                        iter.num a.size2 <| fun k ->
                            u[i,j] <== u[i,j] + a[i,k] * b[k,j]
            static member matmul (u:complex2,a:complex2,b:float2) =
                u.clear()
                iter.num a.size1 <| fun i ->
                    iter.num b.size2 <| fun j ->
                        iter.num a.size2 <| fun k ->
                            u[i,j] <== u[i,j] + a[i,k] * b[k,j]
            static member matmul (u:complex2,a:float2,b:complex2) =
                u.clear()
                iter.num a.size1 <| fun i ->
                    iter.num b.size2 <| fun j ->
                        iter.num a.size2 <| fun k ->
                            u[i,j] <== u[i,j] + a[i,k] * b[k,j]
            static member matmul (u:complex2,a:complex2,b:complex2) =
                u.clear()
                iter.num a.size1 <| fun i ->
                    iter.num b.size2 <| fun j ->
                        iter.num a.size2 <| fun k ->
                            u[i,j] <== u[i,j] + a[i,k] * b[k,j]

            /// <summary>
            /// 行列a×行列bの計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member matmul (a:float2,b:float2) = fun code ->
                ch.d2 a.size1 b.size2 <| fun u ->
                    La.matmul (u,a,b)
                    code u
            /// <summary>
            /// 行列a×行列bの計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member matmul (a:complex2,b:float2) = fun code ->
                ch.z2 a.size1 b.size2 <| fun u ->
                    La.matmul (u,a,b)
                    code u
            /// <summary>
            /// 行列a×行列bの計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member matmul (a:float2,b:complex2) = fun code ->
                ch.z2 a.size1 b.size2 <| fun u ->
                    La.matmul (u,a,b)
                    code u
            /// <summary>
            /// 行列a×行列bの計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member matmul (a:complex2,b:complex2) = fun code ->
                ch.z2 a.size1 b.size2 <| fun u ->
                    La.matmul (u,a,b)
                    code u
                    
            /// <summary>
            /// ベクトルの内積計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member dot (x:complex0,a:complex1,b:complex1) =
                x.clear()
                iter.num a.size1 <| fun j ->
                    x <== x + asm.conj(a[j]) * b[j]
            /// <summary>
            /// ベクトルの内積計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member dot (x:complex0,a:float1,b:float1) =
                x.clear()
                iter.num a.size1 <| fun j ->
                    x <== x + a[j] * b[j]
            /// <summary>
            /// ベクトルの内積計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member dot (x:float0,a:float1,b:float1) =
                x.clear()
                iter.num a.size1 <| fun j ->
                    x <== x + a[j] * b[j]
            /// <summary>
            /// ベクトルの内積計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member dot (a:complex1,b:complex1) = fun code ->
                ch.z <| fun x ->
                    La.dot (x,a,b)
                    code x
            /// <summary>
            /// ベクトルの内積計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="b"></param>
            static member dot (a:float1,b:float1) = fun code ->
                ch.d <| fun x ->
                    La.dot (x,a,b)
                    code x
                    
            /// <summary>
            /// ベクトルのノルム計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="code"></param>
            static member norm (a:float1) = fun code ->
                La.dot (a,a) code
            /// <summary>
            /// ベクトルのノルム計算
            /// </summary>
            /// <param name="a"></param>
            /// <param name="code"></param>
            static member norm (a:complex1) = fun code ->
                La.dot (a,a) code
                
            /// <summary>
            /// ベクトルの規格化
            /// </summary>
            /// <param name="a"></param>
            static member normalize (a:float1) =
                La.dot (a,a) <| fun c ->
                    let dd = a/asm.sqrt(c)
                    a <== a/asm.sqrt(c)
                    
            ///<summary>連立方程式の求解</summary>
            ///<param name="matrix">係数行列</param>
            ///<param name="y">定数項ベクトル→解ベクトル</param>
            static member solve_simuleq (matrix:float2,y:float1) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "連立方程式の求解" <| fun () ->
                    ch.iii <| fun (N,b,info) ->
                        N <== matrix.size1
                        b <== 1
                        ch.i1 N <| fun ipiv ->
                        match p.lang with
                        |F -> 
                            p.param.codewrite("call dgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |C -> 
                            p.param.extern_("void dgesv_(int *n, int *nrhs, double *a, int *lda, int *ipiv, double *b, int *ldb, int *info)")
                            p.param.codewrite("dgesv_(&"+N.code+","+"&"+b.code+","+matrix.code+",&"+N.code+","+ipiv.code+","+y.code+",&"+N.code+",&"+info.code+")"+";\n")
                        |T -> 
                            p.param.codewrite("dgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |H -> 
                            p.param.codewrite("<math>"+y.code+"<mo>&larr;</mo>"+"<msup>"+matrix.code+"<mn>-1</mn></msup>"+y.code+"</math>"+"\n<br/>\n")
                                
            ///<summary>連立方程式の求解</summary>
            ///<param name="matrix">係数行列</param>
            ///<param name="y">定数項ベクトル→解ベクトル</param>
            static member solve_simuleq (matrix:complex2,y:complex1) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "連立方程式の求解" <| fun () ->
                    ch.iii <| fun (N,b,info) ->
                        N <== matrix.size1
                        b <== 1
                        ch.i1 N <| fun ipiv ->
                        match p.lang with
                        |F -> 
                            p.param.codewrite("call zgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |C -> 
                            p.param.extern_("void zgesv_(int *n, int *nrhs, double complex *a, int *lda, int *ipiv, double complex *b, int *ldb, int *info)")
                            p.param.codewrite("zgesv_(&"+N.code+","+"&"+b.code+","+matrix.code+",&"+N.code+","+ipiv.code+","+y.code+",&"+N.code+",&"+info.code+")"+";\n")
                        |T -> 
                            p.param.codewrite("zgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |H -> 
                            p.param.codewrite("<math>"+y.code+"<mo>&larr;</mo>"+"<msup>"+matrix.code+"<mn>-1</mn></msup>"+y.code+"</math>"+"\n<br/>\n")

            ///<summary>連立方程式の求解</summary>
            ///<param name="matrix">係数行列</param>
            ///<param name="y">定数項ベクトルを列方向に並べた配列→解ベクトルを列方向に並べた配列</param>
            static member solve_simuleqs (matrix:float2,y:float2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "連立方程式の求解" <| fun () ->
                    ch.iii <| fun (N,b,info) ->
                        N <== matrix.size1
                        b <== y.size2
                        ch.i1 N <| fun ipiv ->
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call dgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                            |C -> 
                                p.param.extern_("void dgesv_(int *n, int *nrhs, double *a, int *lda, int *ipiv, double *b, int *ldb, int *info)")
                                p.param.codewrite("dgesv_(&"+N.code+","+"&"+b.code+","+matrix.code+",&"+N.code+","+ipiv.code+","+y.code+",&"+N.code+",&"+info.code+")"+";\n")
                            |T -> 
                                p.param.codewrite("dgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                            |H -> 
                                p.param.codewrite("<math>"+y.code+"<mo>&larr;</mo>"+"<msup>"+matrix.code+"<mn>-1</mn></msup>"+y.code+"</math>"+"\n<br/>\n")
            ///<summary>連立方程式の求解</summary>
            ///<param name="matrix">係数行列</param>
            ///<param name="y">定数項ベクトルを列方向に並べた配列→解ベクトルを列方向に並べた配列</param>
            static member solve_simuleqs (matrix:complex2,y:complex2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "連立方程式の求解" <| fun () ->
                    ch.iii <| fun (N,b,info) ->
                        N <== matrix.size1
                        b <== y.size2
                        ch.i1 N <| fun ipiv ->
                        match p.lang with
                        |F -> 
                            p.param.codewrite("call zgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |C -> 
                            p.param.extern_("void zgesv_(int *n, int *nrhs, double complex *a, int *lda, int *ipiv, double complex *b, int *ldb, int *info)")
                            p.param.codewrite("zgesv_(&"+N.code+","+"&"+b.code+","+matrix.code+",&"+N.code+","+ipiv.code+","+y.code+",&"+N.code+",&"+info.code+")"+";\n")
                        |T -> 
                            p.param.codewrite("zgesv("+N.code+","+b.code+","+matrix.code+","+N.code+","+ipiv.code+","+y.code+","+N.code+","+info.code+")"+"\n")
                        |H -> 
                            p.param.codewrite("<math>"+y.code+"<mo>&larr;</mo>"+"<msup>"+matrix.code+"<mn>-1</mn></msup>"+y.code+"</math>"+"\n<br/>\n")
                                    
            ///<summary>逆行列の計算</summary>
            ///<param name="mat1">元の行列</param>
            ///<param name="mat2">mat1の逆行列</param>
            static member inverse_matrix (mat2:float2,mat1:float2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "逆行列の計算" <| fun () ->
                    mat2.clear()
                    iter.num mat1.size1 <| fun i -> mat2[i,i] <== 1.0
                    ch.ii <| fun (npre,info) ->
                        npre<==mat1.size1
                        ch.i1 npre <| fun ipiv ->
                            ipiv.clear()
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call dgesv("+npre.code+", "+npre.code+","+mat1.code+", "+npre.code+", "+ipiv.code+","+mat2.code+", "+npre.code+", "+info.code+")")
                            |C -> 
                                p.param.extern_("void dgesv_(int *n, int *nrhs, double *a, int *lda, int *ipiv, double *b, int *ldb, int *info)")
                                p.param.codewrite("dgesv_(&"+npre.code+","+"&"+npre.code+", "+mat1.code+", &"+npre.code+", "+ipiv.code+", *"+mat2.code+", &"+npre.code+", &"+info.code+");")
                            |T -> 
                                p.param.codewrite("call dgesv("+npre.code+", "+npre.code+","+mat1.code+", "+npre.code+", "+ipiv.code+","+mat2.code+", "+npre.code+", "+info.code+")")
                            |H -> 
                                p.param.codewrite("<math>"+mat2.code+"<mo>&larr;</mo>"+"<msup>"+mat1.code+"<mn>-1</mn></msup>"+"</math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () ->
                                print.c "InvMatrix Info: "
                                print.c info
                                print.br
                            
            ///<summary>逆行列の計算</summary>
            ///<param name="mat1">元の行列</param>
            ///<param name="mat2">mat1の逆行列</param>
            static member inverse_matrix (mat2:complex2,mat1:complex2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "逆行列の計算" <| fun () ->
                    mat2.clear()
                    iter.num mat1.size1 <| fun i -> mat2[i,i] <== 1.0
                    ch.ii <| fun (npre,info) ->
                        npre<==mat1.size1
                        ch.i1 npre <| fun ipiv ->
                            ipiv.clear()
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call zgesv("+npre.code+", "+npre.code+","+mat1.code+", "+npre.code+", "+ipiv.code+","+mat2.code+", "+npre.code+", "+info.code+")")
                            |C -> 
                                p.param.extern_("void zgesv_(int *n, int *nrhs, double complex *a, int *lda, int *ipiv, double complex *b, int *ldb, int *info)")
                                p.param.codewrite("zgesv_(&"+npre.code+","+"&"+npre.code+", "+mat1.code+", &"+npre.code+", "+ipiv.code+", *"+mat2.code+", &"+npre.code+", &"+info.code+");")
                            |T -> 
                                p.param.codewrite("call zgesv("+npre.code+", "+npre.code+","+mat1.code+", "+npre.code+", "+ipiv.code+","+mat2.code+", "+npre.code+", "+info.code+")")
                            |H -> 
                                p.param.codewrite("<math>"+mat2.code+"<mo>&larr;</mo>"+"<msup>"+mat1.code+"<mn>-1</mn></msup>"+"</math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () ->
                                print.c "InvMatrix Info: "
                                print.c info
                                print.br
                            
            ///<summary>行列の階数</summary>
            static member rank (rank:int0,mat:float2,cond:float0) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "行列の階数" <| fun () ->
                    ch.iii <| fun (npre,info,lwork) ->
                        npre<==mat.size1
                        ch.i1 npre <| fun ipiv -> ch.d1 npre <| fun s -> ch.z2 npre npre <| fun u -> ch.z2 npre npre <| fun vt -> ch.d1 (5*npre) <| fun rwork -> ch.i1 (8*npre) <| fun iwork -> 
                            ipiv.clear()
                            lwork <== 2*npre+npre
                            ch.d1 lwork <| fun work ->
                            //特異値分解を利用
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call dgesdd('N', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |C -> 
                                    p.param.extern_("void dgesdd_(char jobz, int m, int n, double *a, int lda, double *s, double *u, int ldu, double *vt, int ldvt, double *work, int lwork, int *iwork, int info)")
                                    p.param.codewrite("dgesdd_('N', &" +  npre.code + "," + "&" + npre.code + ", " + mat.code+", &" + npre.code + ", *" + s.code + ", *" + u.code + ", &" + npre.code + ", *" + vt.code + ", &" + npre.code + ", &" + work.code + ", &" + lwork.code + ", *" + rwork.code + ", *" + iwork.code + ", &" + info.code + ");")
                            |T -> 
                                p.param.codewrite("call dgesdd('N', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |H -> 
                                p.param.codewrite("<math>"+rank.code+"<mo>&larr;</mo>"+"<mi>rank</mi><mo>[</mo>"+mat.code+"<mo>]</mo></math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () -> 
                                print.c "rank Info: "
                                print.c info
                                print.br
                            rank.clear()
                            iter.array s <| fun i -> 
                                br.if1 (s[i] >. cond) <| fun () -> rank.inc
                                
            ///<summary>行列の階数</summary>
            static member rank (rank:int0,mat:complex2,cond:float0) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "行列の階数" <| fun () ->
                    ch.iii <| fun (npre,info,lwork) ->
                        npre<==mat.size1
                        ch.i1 npre <| fun ipiv -> ch.d1 npre <| fun s -> ch.z2 npre npre <| fun u -> ch.z2 npre npre <| fun vt -> ch.d1 (5*npre) <| fun rwork -> ch.i1 (8*npre) <| fun iwork -> 
                            ipiv.clear()
                            lwork <== 2*npre+npre
                            ch.z1 lwork <| fun work ->
                            //特異値分解を利用
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call zgesdd('N', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |C -> 
                                  p.param.extern_("void zgesdd_(char jobz, int m, int n, double complex *a, int lda, double *s, double complex *u, int ldu, double complex *vt, int ldvt, double complex *work, int lwork, double *rwork, int *iwork, int info)")
                                  p.param.codewrite("zgesdd_('N', &" +  npre.code + "," + "&" + npre.code + ", " + mat.code+", &" + npre.code + ", *" + s.code + ", *" + u.code + ", &" + npre.code + ", *" + vt.code + ", &" + npre.code + ", &" + work.code + ", &" + lwork.code + ", *" + rwork.code + ", *" + iwork.code + ", &" + info.code + ");")
                            |T -> 
                                p.param.codewrite("call zgesdd('N', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |H -> 
                                p.param.codewrite("<math>"+rank.code+"<mo>&larr;</mo>"+"<mi>rank</mi><mo>[</mo>"+mat.code+"<mo>]</mo></math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () ->
                                print.c "rank Info: "
                                print.c info
                                print.br
                            rank.clear()
                            iter.array s <| fun i -> 
                                br.if1 (s[i] >. cond) <| fun () -> rank.inc
                                
            static member inverse_matrix2 (mat2:complex2,mat:float2,cond:float0) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "疑似逆行列" <| fun () ->
                    ch.iii <| fun (npre,info,lwork) ->
                        npre<==mat.size1
                        ch.i1 npre <| fun ipiv -> ch.d1 npre <| fun s -> ch.z2 npre npre <| fun u -> ch.z2 npre npre <| fun u2 -> ch.z2 npre npre <| fun vt -> ch.d1 (npre*(5*npre+7)) <| fun rwork -> ch.i1 (8*npre) <| fun iwork -> 
                            ipiv.clear()
                            lwork <== npre*npre+2*npre+npre
                            ch.d1 lwork <| fun work ->
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call dgesdd('A', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |C -> 
                                p.param.extern_("void dgesdd_(char jobz, int m, int n, double *a, int lda, double *s, double *u, int ldu, double *vt, int ldvt, double *work, int lwork, int *iwork, int info)")
                                p.param.codewrite("dgesdd_('A', &" +  npre.code + "," + "&" + npre.code + ", " + mat.code+", &" + npre.code + ", *" + s.code + ", *" + u.code + ", &" + npre.code + ", *" + vt.code + ", &" + npre.code + ", &" + work.code + ", &" + lwork.code + ", *" + rwork.code + ", *" + iwork.code + ", &" + info.code + ");")
                            |T -> 
                                p.param.codewrite("call dgesdd('A', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |H -> 
                                p.param.codewrite("<math>"+mat2.code+"<mo>&larr;</mo>"+"<msup>"+mat.code+"<mn>-1</mn></msup>"+"</math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () -> 
                                print.c "rank Info: "
                                print.c info
                                print.br
                            //特異値分解した行列をもとに、疑似逆行列は (v^*)×(s^-1)×(u^*)
                            iter.array s <| fun i -> 
                                iter.array s <| fun j -> 
                                    br.if2 (s[i] >. cond)
                                      (fun () ->
                                        u2[i,j]<==(asm.conj(u[j,i])/s[i]))
                                      (fun () ->
                                        //condより小さい特異値は無視
                                        u2[i,j].clear())
                            mat2.clear()
                            iter.array s <| fun i -> 
                                iter.array s <| fun j ->
                                    iter.array s <| fun p ->
                                        mat2[i,j] <== mat2[i,j] + asm.conj(vt[p,i])*u2[p,j]
            static member inverse_matrix2 (mat2:complex2,mat:complex2,cond:float0) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "疑似逆行列" <| fun () ->
                    ch.iii <| fun (npre,info,lwork) ->
                        npre<==mat.size1
                        ch.i1 npre <| fun ipiv -> ch.d1 npre <| fun s -> ch.z2 npre npre <| fun u -> ch.z2 npre npre <| fun u2 -> ch.z2 npre npre <| fun vt -> ch.d1 (npre*(5*npre+7)) <| fun rwork -> ch.i1 (8*npre) <| fun iwork -> 
                            ipiv.clear()
                            lwork <== npre*npre+2*npre+npre
                            ch.z1 lwork <| fun work ->
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call zgesdd('A', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |C -> 
                                p.param.extern_("void zgesdd_(char jobz, int m, int n, double complex *a, int lda, double *s, double complex *u, int ldu, double complex *vt, int ldvt, double complex *work, int lwork, double *rwork, int *iwork, int info)")
                                p.param.codewrite("zgesdd_('A', &" +  npre.code + "," + "&" + npre.code + ", " + mat.code+", &" + npre.code + ", *" + s.code + ", *" + u.code + ", &" + npre.code + ", *" + vt.code + ", &" + npre.code + ", &" + work.code + ", &" + lwork.code + ", *" + rwork.code + ", *" + iwork.code + ", &" + info.code + ");")
                            |T -> 
                                p.param.codewrite("call zgesdd('A', " + npre.code + "," + " " + npre.code + ","  + mat.code+", "  + npre.code + ", "  + s.code + ","   + u.code + ", "  + npre.code + ","   + vt.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ","   + rwork.code + ","   + iwork.code + ", "  + info.code + ")")
                            |H -> 
                                p.param.codewrite("<math>"+mat2.code+"<mo>&larr;</mo>"+"<msup>"+mat.code+"<mn>-1</mn></msup>"+"</math>"+"\n<br/>\n")
                            br.if1 (info =/. 0) <| fun () -> 
                                print.c "rank Info: "
                                print.c info
                                print.br
                            //特異値分解した行列をもとに、疑似逆行列は (v^*)×(s^-1)×(u^*)
                            iter.array s <| fun i -> 
                                iter.array s <| fun j -> 
                                    br.if2 (s[i] >. cond)
                                        <| fun () ->
                                            u2[i,j]<==(asm.conj(u[j,i])/s[i])
                                        <| fun () ->
                                            //condより小さい特異値は無視
                                            u2[i,j].clear()
                            mat2.clear()
                            iter.array s <| fun i -> 
                                iter.array s <| fun j ->
                                    iter.array s <| fun p ->
                                        mat2[i,j] <== mat2[i,j] + asm.conj(vt[p,i])*u2[p,j]
                                        
            /// <summary>
            /// Ax=λxの固有値λと固有ベクトルxを計算
            /// </summary>
            /// <param name="eigenvalues">固有値</param>
            /// <param name="eigenvectors">固有ベクトル</param>
            /// <param name="mat1">複素非対称行列</param>
            static member eigen_matrix (eigenvalues:complex1,eigenvectors:float2,mat1:num2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "非対称実行列の固有値" <| fun () ->
                    eigenvectors.clear()
                    ch.d1 eigenvectors.size1 <| fun eigenvalues_re ->
                    ch.d1 eigenvectors.size1 <| fun eigenvalues_im ->
                    ch.iii <| fun (npre,ldvldummy,info) ->
                            npre<==mat1.size1
                            ch.d2 _1 _1 <| fun dummy ->
                                ch.i <| fun lwork ->
                                    lwork <== 4*npre
                                    ch.d1 lwork <| fun work ->
                                            eigenvalues.clear()
                                            ldvldummy <== 1
                                            match p.lang with
                                            |F -> 
                                                p.param.codewrite("call dgeev('No left vectors', 'Vectors (right)', "    + npre.code + ", "   + mat1.code + ", "  + npre.code + ", "  + eigenvalues_re.code + ", "  + eigenvalues_im.code + ","   + dummy.code + ",  " + ldvldummy.code + ", "  + eigenvectors.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ", " + info.code + ")")
                                            |C -> 
                                                p.param.extern_("void dgeev_(char *, char *, int *, double *, int *, double *, double *, int *, double *, int *, double *, int *, double *, int *)")
                                                p.param.codewrite("dgeev_('N', 'V', &" + npre.code + ", "  + mat1.code + ", &" + npre.code + ", " + eigenvalues_re.code + ", " + eigenvalues_im.code + ", " + dummy.code + ", &" + ldvldummy.code + ", " + eigenvectors.code + ", &" + npre.code + ", " + work.code + ", &" + lwork.code + ", &" + info.code + ");")
                                            |T -> 
                                                p.param.codewrite("call dgeev('No left vectors', 'Vectors (right)', "    + npre.code + ", "   + mat1.code + ", "  + npre.code + ", "  + eigenvalues.code + ","   + dummy.code + ",  " + ldvldummy.code + ", "  + eigenvectors.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ", "  + info.code + ")")
                                            |H -> 
                                                p.param.codewrite("<math><mi>solve</mi><mspace width=\"0.5em\" />"+mat1.code+eigenvectors.code+"<mo>=</mo>"+eigenvalues.code+eigenvectors.code+"</math>"+"\n<br/>\n")
                                            br.if1 (info =/. 0) <| fun () ->
                                                print.c "Eigenvalue Info: "
                                                print.c info
                                                print.br
                            eigenvalues.foreach <| fun i -> eigenvalues[i] <== eigenvalues_re[i] + asm.uj * eigenvalues_im[i]
            /// <summary>
            /// Ax=λxの固有値λと固有ベクトルxを計算
            /// </summary>
            /// <param name="eigenvalues">固有値</param>
            /// <param name="eigenvectors">固有ベクトル</param>
            /// <param name="mat1">複素非対称行列</param>
            static member eigen_matrix (eigenvalues:complex1,eigenvectors:complex2,mat1:num2) =
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "非対称複素行列の固有値" <| fun () ->
                    eigenvectors.clear()
                    ch.iii <| fun (npre,ldvldummy,info) ->
                            npre<==mat1.size1
                            ch.z2 _1 _1 <| fun dummy ->
                                ch.i <| fun lwork ->
                                    lwork <== 2*npre
                                    ch.z1 lwork <| fun work ->
                                        ch.d1 (2*npre) <| fun rwork ->
                                            eigenvalues.clear()
                                            ldvldummy <== 1
                                            match p.lang with
                                            |F -> 
                                                p.param.codewrite("call zgeev('No left vectors', 'Vectors (right)', "    + npre.code + ", "   + mat1.code + ", "  + npre.code + ", "  + eigenvalues.code + ","   + dummy.code + ",  " + ldvldummy.code + ", "  + eigenvectors.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ", "  + rwork.code + ", "  + info.code + ")")
                                            |C -> 
                                                p.param.extern_("void zgeev_(char *, char *, int *, double complex *, int *, double complex *, double complex *, int *, double complex *, int *, double complex *, int *, double *, int *)")
                                                p.param.codewrite("zgeev_('N', 'V', &" + npre.code + ", "  + mat1.code + ", &" + npre.code + ", " + eigenvalues.code + ", " + dummy.code + ", &" + ldvldummy.code + ", " + eigenvectors.code + ", &" + npre.code + ", " + work.code + ", &" + lwork.code + ", " + rwork.code + ", &" + info.code + ");")
                                            |T -> 
                                                p.param.codewrite("call zgeev('No left vectors', 'Vectors (right)', "    + npre.code + ", "   + mat1.code + ", "  + npre.code + ", "  + eigenvalues.code + ","   + dummy.code + ",  " + ldvldummy.code + ", "  + eigenvectors.code + ", "  + npre.code + ", "  + work.code + ", "  + lwork.code + ", "  + rwork.code + ", "  + info.code + ")")
                                            |H -> 
                                                p.param.codewrite("<math><mi>solve</mi><mspace width=\"0.5em\" />"+mat1.code+eigenvectors.code+"<mo>=</mo>"+eigenvalues.code+eigenvectors.code+"</math>"+"\n<br/>\n")
                                            br.if1 (info =/. 0) <| fun () ->
                                                print.c "Eigenvalue Info: "
                                                print.c info
                                                print.br
            /// <summary>
            /// Ax=λBxの固有値λと固有ベクトルxを計算
            /// </summary>
            /// <param name="eigenvalues1">λ1(λ=λ1/λ2)</param>
            /// <param name="eigenvalues2">λ2(λ=λ1/λ2)</param>
            /// <param name="eigenvectors">固有ベクトルx</param>
            /// <param name="mat1">行列A</param>
            /// <param name="mat2">行列B</param>
            static member eigen_matrix2 (eigenvalues1:complex1,eigenvalues2:complex1,eigenvectors:complex2,mat1:complex2,mat2:complex2) =
                    codestr.section "非対称複素行列の一般化固有値" <| fun () ->
                        p.param.option_("-llapack")
                        p.param.option_("-lblas")
                        eigenvectors.clear()
                        ch.iii <| fun (npre,ldvldummy,info) ->
                            npre<==mat1.size1
                            ch.z2 _1 _1 <| fun dummy ->
                                ch.i <| fun lwork ->
                                    lwork <== npre + 64 * npre
                                    ch.z1 lwork <| fun work ->
                                        ch.d1 (8*npre) <| fun rwork ->
                                            eigenvalues1.clear()
                                            eigenvalues2.clear()
                                            ldvldummy <== 1
                                            match p.lang with
                                            |F -> 
                                                p.param.codewrite("call zggev("+
                                                    "'N'" + ", " +
                                                    "'V'" + ", " + 
                                                    npre.code + ", " + 
                                                    mat1.code + ", " + 
                                                    npre.code + ", " + 
                                                    mat2.code + ", " + 
                                                    npre.code + ", " + 
                                                    eigenvalues1.code + ", " + 
                                                    eigenvalues2.code + "," + 
                                                    dummy.code + ",  " + 
                                                    ldvldummy.code + ", " +
                                                    eigenvectors.code + ", " + 
                                                    npre.code + ", "  + 
                                                    work.code + ", "  + 
                                                    lwork.code + ", "  + 
                                                    rwork.code + ", "  + 
                                                    info.code + ")")
                                            |C -> 
                                                  p.param.extern_("void zggev_(char *, char *, int *, double complex *, int *, double complex *, int *, double complex *, double complex *, double complex *, int *, double complex *, int *, double complex *, int *, double *, int *)")
                                                  p.param.codewrite("zggev_(" +
                                                      "'N', " +
                                                      "'V', " +
                                                      "&" + npre.code + ", " +
                                                      mat1.code + ", " +
                                                      "&" + npre.code + ", " +
                                                      mat2.code + ", " +
                                                      "&" + npre.code + ", " +
                                                      eigenvalues1.code + ", " +
                                                      eigenvalues2.code + ", " +
                                                      dummy.code + ", "+
                                                      "&" + ldvldummy.code + ", " +
                                                      eigenvectors.code + ", "+
                                                      "&" + npre.code + ", " +
                                                      work.code + ", " +
                                                      "&" + lwork.code + ", " +
                                                      rwork.code + ", " +
                                                      "&" + info.code + ");")
                                            |T -> 
                                                p.param.codewrite("call zggev("+
                                                    "'N'" + ", " +
                                                    "'V'" + ", " + 
                                                    npre.code + ", " + 
                                                    mat1.code + ", " + 
                                                    npre.code + ", " + 
                                                    mat2.code + ", " + 
                                                    npre.code + ", " + 
                                                    eigenvalues1.code + ", " + 
                                                    eigenvalues2.code + "," + 
                                                    dummy.code + ",  " + 
                                                    ldvldummy.code + ", " +
                                                    eigenvectors.code + ", " + 
                                                    npre.code + ", "  + 
                                                    work.code + ", "  + 
                                                    lwork.code + ", "  + 
                                                    rwork.code + ", "  + 
                                                    info.code + ")")
                                            |H -> 
                                                p.param.codewrite("<math><mi>solve</mi><mspace width=\"0.5em\" />"+mat1.code+"<mi>&chi;</mi>"+"<mo>=</mo>"+"<mi>&lambda;</mi>"+mat2.code+"<mi>&chi;</mi></math>"+"\n<br/>\n")
                                                p.param.codewrite("<math><mi>&lambda;</mi><mo>=</mo>"+"<mfrac><mrow>"+eigenvalues1.code+"</mrow><mrow>"+eigenvalues2.code+"</mrow></mfrac></math>"+"\n<br/>\n")
                                            br.if1 (info =/. 0) <| fun () ->
                                                print.c "Eigenvalue Info: "
                                                print.c info
                                                print.br
            /// <summary>
            /// Ax=λBxの固有値λと固有ベクトルxを計算
            /// </summary>
            /// <param name="eigenvalues1">λ1(λ=λ1/λ2)</param>
            /// <param name="eigenvalues2">λ2(λ=λ1/λ2)</param>
            /// <param name="eigenvectors">固有ベクトルx</param>
            /// <param name="mat1">行列A</param>
            /// <param name="mat2">行列B</param>
            static member eigen_matrix2 (eigenvalues1:complex1,eigenvalues2:float1,eigenvectors:float2,mat1:float2,mat2:float2) =
                    codestr.section "非対称複素行列の一般化固有値" <| fun () ->
                        p.param.option_("-llapack")
                        p.param.option_("-lblas")
                        eigenvectors.clear()
                        ch.d1 eigenvalues1.size1 <| fun eigenvalues1re ->
                        ch.d1 eigenvalues1.size1 <| fun eigenvalues1im ->
                        ch.iii <| fun (npre,ldvldummy,info) ->
                            npre<==mat1.size1
                            ch.d2 _1 _1 <| fun dummy ->
                                ch.i <| fun lwork ->
                                    lwork <== npre + 64 * npre
                                    ch.d1 lwork <| fun work ->
                                            eigenvalues1re.clear()
                                            eigenvalues1im.clear()
                                            eigenvalues2.clear()
                                            ldvldummy <== 1
                                            match p.lang with
                                            |F -> 
                                                p.param.codewrite("call dggev("+
                                                    "'N'" + ", " +
                                                    "'V'" + ", " + 
                                                    npre.code + ", " + 
                                                    mat1.code + ", " + 
                                                    npre.code + ", " + 
                                                    mat2.code + ", " + 
                                                    npre.code + ", " + 
                                                    eigenvalues1re.code + ", " + 
                                                    eigenvalues1im.code + ", " + 
                                                    eigenvalues2.code + "," + 
                                                    dummy.code + ",  " + 
                                                    ldvldummy.code + ", " +
                                                    eigenvectors.code + ", " + 
                                                    npre.code + ", "  + 
                                                    work.code + ", "  + 
                                                    lwork.code + ", "  + 
                                                    info.code + ")")
                                            |C -> 
                                                  p.param.extern_("void dggev_(char *, char *, int *, double complex *, int *, double complex *, int *, double complex *, double complex *, double complex *, int *, double complex *, int *, double complex *, int *, double *, int *)")
                                                  p.param.codewrite("dggev_(" +
                                                      "'N', " +
                                                      "'V', " +
                                                      "&" + npre.code + ", " +
                                                      mat1.code + ", " +
                                                      "&" + npre.code + ", " +
                                                      mat2.code + ", " +
                                                      "&" + npre.code + ", " +
                                                      eigenvalues1re.code + ", " +
                                                      eigenvalues1im.code + ", " +
                                                      eigenvalues2.code + ", " +
                                                      dummy.code + ", "+
                                                      "&" + ldvldummy.code + ", " +
                                                      eigenvectors.code + ", "+
                                                      "&" + npre.code + ", " +
                                                      work.code + ", " +
                                                      "&" + lwork.code + ", " +
                                                      "&" + info.code + ");")
                                            |T -> 
                                                p.param.codewrite("call dggev("+
                                                    "'N'" + ", " +
                                                    "'V'" + ", " + 
                                                    npre.code + ", " + 
                                                    mat1.code + ", " + 
                                                    npre.code + ", " + 
                                                    mat2.code + ", " + 
                                                    npre.code + ", " + 
                                                    eigenvalues1re.code + ", " + 
                                                    eigenvalues1im.code + ", " + 
                                                    eigenvalues2.code + "," + 
                                                    dummy.code + ",  " + 
                                                    ldvldummy.code + ", " +
                                                    eigenvectors.code + ", " + 
                                                    npre.code + ", "  + 
                                                    work.code + ", "  + 
                                                    lwork.code + ", "  + 
                                                    info.code + ")")
                                            |H -> 
                                                p.param.codewrite("<math><mi>solve</mi><mspace width=\"0.5em\" />"+mat1.code+"<mi>&chi;</mi>"+"<mo>=</mo>"+"<mi>&lambda;</mi>"+mat2.code+"<mi>&chi;</mi></math>"+"\n<br/>\n")
                                                p.param.codewrite("<math><mi>&lambda;</mi><mo>=</mo>"+"<mfrac><mrow>"+eigenvalues1re.code+"<mo>+</mo></mi>j<mi>"+eigenvalues1im.code+"</mrow><mrow>"+eigenvalues2.code+"</mrow></mfrac></math>"+"\n<br/>\n")
                                            br.if1 (info =/. 0) <| fun () ->
                                                print.c "Eigenvalue Info: "
                                                print.c info
                                                print.br
                            iter.num eigenvalues1.size1 <| fun i ->
                                eigenvalues1[i] <== eigenvalues1re[i] + asm.uj * eigenvalues1im[i]
                                            
            /// <summary>
            /// 連立方程式の求解(Tikhonovの正則化法)
            /// </summary>
            /// <param name="fu_mat">係数行列</param>
            /// <param name="fu_cst">定数項ベクトル</param>
            /// <param name="code">解に対して行う処理</param>
            static member solve_simuleq_t(fu_mat:float2,fu_cst:float1) = fun code ->
                codestr.h2 "連立方程式の求解(Tikhonovの正則化法)" <| fun () ->
                    ch.d2 fu_mat.size2 fu_mat.size2 <| fun FF ->
                    ch.d1 fu_mat.size2 <| fun bb ->
                        let lambda = 1E-6 //正則化パラメータ
                        
                        FF.clear()
                        //FF = fu_mat^T * fu_mat
                        ch.d <| fun tmp ->
                            iter.num FF.size1 <| fun i ->
                                iter.num FF.size2 <| fun j ->
                                    tmp.clear()
                                    iter.num fu_mat.size1 <| fun k ->
                                        tmp <== tmp + fu_mat[k,i]*fu_mat[k,j]
                                    FF[i,j] <== tmp
                                    
                        //FF = fu_mat^T * fu_mat + λ^2 * I
                        iter.num FF.size1 <| fun i ->
                                FF[i,i] <== FF[i,i] + lambda * lambda
                        //bb = fu_mat^T * fu_cst
                        ch.d <| fun tmp ->
                            bb.clear()
                            iter.num bb.size1 <| fun i ->
                                tmp.clear()
                                iter.num fu_cst.size1 <| fun k ->
                                    tmp <== tmp + fu_mat[k,i]*fu_cst[k]
                                bb[i] <== tmp
                        La.solve_simuleq(FF,bb)
                        code(bb)
            /// <summary>
            /// 連立方程式の求解(Tikhonovの正則化法)
            /// </summary>
            /// <param name="fu_mat">係数行列</param>
            /// <param name="fu_cst">定数項ベクトル</param>
            /// <param name="code">解に対して行う処理</param>
            static member solve_simuleq_t(fu_mat:complex2,fu_cst:complex1) = fun code ->
                codestr.h2 "連立方程式の求解(Tikhonovの正則化法)" <| fun () ->
                    ch.z2 fu_mat.size2 fu_mat.size2 <| fun FF ->
                    ch.z1 fu_mat.size2 <| fun bb ->
                        let lambda = 1E-6 //正則化パラメータ
                        
                        FF.clear()
                        //FF = fu_mat^T * fu_mat
                        ch.z <| fun tmp ->
                            iter.num FF.size1 <| fun i ->
                                iter.num FF.size2 <| fun j ->
                                    tmp.clear()
                                    iter.num fu_mat.size1 <| fun k ->
                                        tmp <== tmp + fu_mat[k,i]*fu_mat[k,j]
                                    FF[i,j] <== tmp
                                    
                        //FF = fu_mat^T * fu_mat + λ^2 * I
                        iter.num FF.size1 <| fun i ->
                                FF[i,i] <== FF[i,i] + lambda * lambda
                        //bb = fu_mat^T * fu_cst
                        ch.z <| fun tmp ->
                            bb.clear()
                            iter.num bb.size1 <| fun i ->
                                tmp.clear()
                                iter.num fu_cst.size1 <| fun k ->
                                    tmp <== tmp + fu_mat[k,i]*fu_cst[k]
                                bb[i] <== tmp
                        La.solve_simuleq(FF,bb)
                        code(bb)

            /// <summary>
            /// 連立方程式の求解(Tikhonovの正則化法)
            /// </summary>
            /// <param name="fu_mat">係数行列</param>
            /// <param name="fu_cst">定数項ベクトル</param>
            /// <param name="lambda">正則化パラメータ</param>
            /// <param name="code">解に対して行う処理</param>
            static member solve_simuleq_tt(fu_mat:float2,fu_cst:float1,lambda:double) = fun code ->
                codestr.h2 "連立方程式の求解(Tikhonovの正則化法)" <| fun () ->
                        ch.d2 fu_mat.size2 fu_mat.size2 <| fun FF ->
                        ch.d1 fu_mat.size2 <| fun bb ->
                            //let lambda = 1E-6 //正則化パラメータ
                            
                            FF.clear()
                            //FF = fu_mat^T * fu_mat
                            ch.d <| fun tmp ->
                                iter.num FF.size1 <| fun i ->
                                    iter.num FF.size2 <| fun j ->
                                        tmp.clear()
                                        iter.num fu_mat.size1 <| fun k ->
                                            tmp <== tmp + fu_mat[k,i]*fu_mat[k,j]
                                        FF[i,j] <== tmp
                                        
                            //FF = fu_mat^T * fu_mat + λ^2 * I
                            iter.num FF.size1 <| fun i ->
                                    FF[i,i] <== FF[i,i] + lambda * lambda
                            //bb = fu_mat^T * fu_cst
                            ch.d <| fun tmp ->
                                bb.clear()
                                iter.num bb.size1 <| fun i ->
                                    tmp.clear()
                                    iter.num fu_cst.size1 <| fun k ->
                                        tmp <== tmp + fu_mat[k,i]*fu_cst[k]
                                    bb[i] <== tmp
                            La.solve_simuleq(FF,bb)
                            code(bb)
            /// <summary>
            /// 連立方程式の求解(Tikhonovの正則化法)
            /// </summary>
            /// <param name="fu_mat">係数行列</param>
            /// <param name="fu_cst">定数項ベクトル</param>
            /// <param name="lambda">正則化パラメータ</param>
            /// <param name="code">解に対して行う処理</param>
            static member solve_simuleq_tt(fu_mat:complex2,fu_cst:complex1,lambda:double) = fun code ->
                codestr.h2 "連立方程式の求解(Tikhonovの正則化法)" <| fun () ->
                        ch.z2 fu_mat.size2 fu_mat.size2 <| fun FF ->
                        ch.z1 fu_mat.size2 <| fun bb ->
                            //let lambda = 1E-6 //正則化パラメータ
                            
                            FF.clear()
                            //FF = fu_mat^T * fu_mat
                            ch.z <| fun tmp ->
                                iter.num FF.size1 <| fun i ->
                                    iter.num FF.size2 <| fun j ->
                                        tmp.clear()
                                        iter.num fu_mat.size1 <| fun k ->
                                            tmp <== tmp + asm.conj(fu_mat[k,i])*fu_mat[k,j]
                                        FF[i,j] <== tmp
                                        
                            //FF = fu_mat^T * fu_mat + λ^2 * I
                            iter.num FF.size1 <| fun i ->
                                    FF[i,i] <== FF[i,i] + lambda * lambda
                            //bb = fu_mat^T * fu_cst
                            ch.z <| fun tmp ->
                                bb.clear()
                                iter.num bb.size1 <| fun i ->
                                    tmp.clear()
                                    iter.num fu_cst.size1 <| fun k ->
                                        tmp <== tmp + asm.conj(fu_mat[k,i])*fu_cst[k]
                                    bb[i] <== tmp
                            La.solve_simuleq(FF,bb)
                            code(bb)

            /// <summary>
            /// 連立方程式の求解(Tikhonovの正則化法)
            /// </summary>
            /// <param name="fu_mat">係数行列</param>
            /// <param name="fu_cst">定数項ベクトル(列サイズ=1)</param>
            /// <param name="lambda">正則化パラメータ</param>
            /// <param name="code">解に対して行う処理</param>
            static member solve_simuleq_tt2(fu_mat:complex2,fu_cst:complex2,lambda:float0) = fun code ->
                codestr.h2 "連立方程式の求解(Tikhonovの正則化法)" <| fun () ->
                    ch.z2 fu_mat.size2 fu_mat.size2 <| fun FF ->
                    ch.z1 fu_mat.size2 <| fun bb ->
                        FF.clear()
                        //FF = fu_mat^T * fu_mat
                        ch.z <| fun tmp ->
                            iter.num FF.size1 <| fun i ->
                                iter.num FF.size2 <| fun j ->
                                    tmp.clear()
                                    iter.num fu_mat.size1 <| fun k ->
                                        tmp <== tmp + asm.conj(fu_mat[k,i])*fu_mat[k,j]
                                    FF[i,j] <== tmp
                        //FF = fu_mat^T * fu_mat + λ^2 * I
                        iter.num FF.size1 <| fun i ->
                                FF[i,i] <== FF[i,i] + lambda * lambda
                        //bb = fu_mat^T * fu_cst
                        ch.z <| fun tmp ->
                            bb.clear()
                            iter.num bb.size1 <| fun i ->
                                tmp.clear()
                                iter.num fu_cst.size1 <| fun k ->
                                    tmp <== tmp + asm.conj(fu_mat[k,i])*fu_cst[k,1]
                                bb[i] <== tmp
                        La.solve_simuleq(FF,bb)
                        code(bb)

            /// <summary>
            /// 行列式の常用対数を計算
            /// </summary>
            /// <param name="matrix">行列</param>
            /// <param name="code">行列式の値を用いて実行するコード</param>
            static member determinant (matrix:float2) = fun code ->
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "行列式の常用対数を計算" <| fun () ->
                    ch.iid <| fun (N,info,d) ->
                        N <== matrix.size1
                        ch.i1 N <| fun ipiv ->
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call dgetrf("+N.code+","+N.code+","+matrix.code+","+N.code+","+ipiv.code+","+info.code+")"+"\n")
                            |C -> 
                                p.param.extern_("void dgetrf_(int *, int *, double complex *, int *, int *, int *)")
                                p.param.codewrite("dgetrf_(&"+N.code+","+"&"+N.code+","+matrix.code+",&"+N.code+","+ipiv.code+",&"+info.code+")"+";\n")
                            |T -> 
                                p.param.codewrite("dgetrf("+N.code+","+N.code+","+matrix.code+","+N.code+","+ipiv.code+","+info.code+")"+"\n")
                            |H -> 
                                p.param.codewrite("<math>"+d.code+"<mo>&larr;</mo>"+"<mo>|</mo>"+matrix.code+"<mo>|</mo>"+"</math>"+"\n<br/>\n")
                        d.clear()
                        iter.num N <| fun i ->
                            d <== d + asm.log10(asm.abs(matrix[i,i]))
                        code(d)
            /// <summary>
            /// 行列式の常用対数を計算
            /// </summary>
            /// <param name="matrix">行列</param>
            /// <param name="code">行列式の値を用いて実行するコード</param>
            static member determinant (matrix:complex2) = fun code ->
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                codestr.section "行列式の常用対数を計算" <| fun () ->
                    ch.iid <| fun (N,info,d) ->
                        N <== matrix.size1
                        ch.i1 N <| fun ipiv ->
                            match p.lang with
                            |F -> 
                                p.param.codewrite("call zgetrf("+N.code+","+N.code+","+matrix.code+","+N.code+","+ipiv.code+","+info.code+")"+"\n")
                            |C -> 
                                p.param.extern_("void zgetrf_(int *, int *, double complex *, int *, int *, int *)")
                                p.param.codewrite("zgetrf_(&"+N.code+","+"&"+N.code+","+matrix.code+",&"+N.code+","+ipiv.code+",&"+info.code+")"+";\n")
                            |T -> 
                                p.param.codewrite("zgetrf("+N.code+","+N.code+","+matrix.code+","+N.code+","+ipiv.code+","+info.code+")"+"\n")
                            |H -> 
                                p.param.codewrite("<math>"+d.code+"<mo>&larr;</mo>"+"<mo>|</mo>"+matrix.code+"<mo>|</mo>"+"</math>"+"\n<br/>\n")
                        d.clear()
                        iter.num N <| fun i ->
                            d <== d + asm.log10(asm.abs(matrix[i,i]))
                        code(d)
                            
            /// <summary>
            /// mat = u * s * v に特異値分解
            /// </summary>
            /// <param name="mat1">複素行列</param>
            /// <param name="u">複素行列u</param>
            /// <param name="s">正方行列sの対角成分</param>
            /// <param name="vt">複素行列vの転置</param>
            static member svd (mat1:float2) = fun (u:float2,s:float1,vt:float2) ->
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                match p.lang with
                  |H ->
                    p.param.codewrite("<math>"+u.code+"<mo>&middot;</mo>"+s.code+"<mo>&middot;</mo>"+"<msup>"+vt.code+"<mi>T</mi></msup><mo>&larr;</mo>"+mat1.code+"</math>\n<br/>\n")
                  |_ ->
                        codestr.section "非対称実行列の特異値分解" <| fun () ->
                            s.clear()
                            u.clear()
                            vt.clear()
                            ch.iiii <| fun (m,n,lda,info) ->
                                m <== mat1.size1
                                n <== mat1.size2
                                ch.ii <| fun (ldu,ldvt) ->
                                ch.i <| fun lwork ->
                                    lwork <== 5*n+n
                                    ch.d1 lwork <| fun work ->
                                    ch.d1 (5*m) <| fun rwork ->
                                        lda <== m
                                        ldu <== u.size1
                                        ldvt <== vt.size2
                                        match p.lang with
                                        |F -> 
                                            p.param.codewrite("call dgesvd("+
                                              "'A', "+
                                              "'A', " + 
                                              m.code + ", " + 
                                              n.code + ", " +
                                              mat1.code + ", " +
                                              lda.code + ", " +
                                              s.code+ ", " + 
                                              u.code + ", "  + 
                                              ldu.code + ", " + 
                                              vt.code + ", "  + 
                                              ldvt.code + ", " + 
                                              work.code + ", " + 
                                              lwork.code + ", " + 
                                              rwork.code + ", " + 
                                              info.code + ")")
                                        |C -> 
                                              p.param.extern_("void dgesvd_(char *, char *, int *, int *, double complex *, int *, double *, double complex *, int *, double complex *, int *, double complex *, int *, double *, int *)")
                                              p.param.codewrite("dgesvd_(" + 
                                                "'A', " + 
                                                "'A', " + 
                                                "&" + m.code + ", " + 
                                                "&" + n.code + ", " +
                                                mat1.code + ", " +
                                                "&" + lda.code + ", " +
                                                s.code+ ", " + 
                                                u.code + ", "  + 
                                                "&" + ldu.code + ", " + 
                                                vt.code + ", "  + 
                                                "&" + ldvt.code + ", " + 
                                                work.code + ", " + 
                                                "&" + lwork.code + ", " + 
                                                rwork.code + ", " + 
                                                "&" + info.code + ");")
                                        |T -> 
                                            p.param.codewrite("dgesvd("+
                                              "'A', "+
                                              "'A', " + 
                                              m.code + ", " + 
                                              n.code + ", " +
                                              mat1.code + ", " +
                                              lda.code + ", " +
                                              s.code+ ", " + 
                                              u.code + ", "  + 
                                              ldu.code + ", " + 
                                              vt.code + ", "  + 
                                              ldvt.code + ", " + 
                                              work.code + ", " + 
                                              lwork.code + ", " + 
                                              rwork.code + ", " + 
                                              info.code + ")")
                                        |H -> 
                                            ()
            /// <summary>
            /// mat = u * s * v に特異値分解
            /// </summary>
            /// <param name="mat1">複素行列</param>
            /// <param name="u">複素行列u</param>
            /// <param name="s">正方行列sの対角成分</param>
            /// <param name="vt">複素行列vの転置</param>
            static member svd (mat1:complex2) = fun (u:complex2,s:float1,vt:complex2) ->
                p.param.option_("-llapack")
                p.param.option_("-lblas")
                match p.lang with
                  |H ->
                    p.param.codewrite("<math>"+u.code+"<mo>&middot;</mo>"+s.code+"<mo>&middot;</mo>"+"<msup>"+vt.code+"<mi>T</mi></msup><mo>&larr;</mo>"+mat1.code+"</math>\n<br/>\n")
                  |_ ->
                        codestr.section "非対称複素行列の特異値分解" <| fun () ->
                            s.clear()
                            u.clear()
                            vt.clear()
                            ch.iiii <| fun (m,n,lda,info) ->
                                m <== mat1.size1
                                n <== mat1.size2
                                ch.ii <| fun (ldu,ldvt) ->
                                ch.i <| fun lwork ->
                                    lwork <== 5*n+n
                                    ch.z1 lwork <| fun work ->
                                    ch.d1 (5*m) <| fun rwork ->
                                        lda <== m
                                        ldu <== u.size1
                                        ldvt <== vt.size2
                                        match p.lang with
                                        |F -> 
                                            p.param.codewrite("call zgesvd("+
                                              "'A', "+
                                              "'A', " + 
                                              m.code + ", " + 
                                              n.code + ", " +
                                              mat1.code + ", " +
                                              lda.code + ", " +
                                              s.code+ ", " + 
                                              u.code + ", "  + 
                                              ldu.code + ", " + 
                                              vt.code + ", "  + 
                                              ldvt.code + ", " + 
                                              work.code + ", " + 
                                              lwork.code + ", " + 
                                              rwork.code + ", " + 
                                              info.code + ")")
                                        |C -> 
                                              p.param.extern_("void zgesvd_(char *, char *, int *, int *, double complex *, int *, double *, double complex *, int *, double complex *, int *, double complex *, int *, double *, int *)")
                                              p.param.codewrite("zgesvd_(" + 
                                                "'A', " + 
                                                "'A', " + 
                                                "&" + m.code + ", " + 
                                                "&" + n.code + ", " +
                                                mat1.code + ", " +
                                                "&" + lda.code + ", " +
                                                s.code+ ", " + 
                                                u.code + ", "  + 
                                                "&" + ldu.code + ", " + 
                                                vt.code + ", "  + 
                                                "&" + ldvt.code + ", " + 
                                                work.code + ", " + 
                                                "&" + lwork.code + ", " + 
                                                rwork.code + ", " + 
                                                "&" + info.code + ");")
                                        |T -> 
                                            p.param.codewrite("zgesvd("+
                                              "'A', "+
                                              "'A', " + 
                                              m.code + ", " + 
                                              n.code + ", " +
                                              mat1.code + ", " +
                                              lda.code + ", " +
                                              s.code+ ", " + 
                                              u.code + ", "  + 
                                              ldu.code + ", " + 
                                              vt.code + ", "  + 
                                              ldvt.code + ", " + 
                                              work.code + ", " + 
                                              lwork.code + ", " + 
                                              rwork.code + ", " + 
                                              info.code + ")")
                                        |H -> 
                                            ()
                                          
            /// <summary>
            /// 連立同次方程式を求解
            /// </summary>
            /// <param name="mat">複素係数行列</param>
            /// <param name="f">連立方程式の解</param>
            static member solve_homogeneq (mat:complex2) (f:complex1) =
                ch.d1 mat.size1 <| fun s ->
                ch.z2 mat.size1 mat.size2 <| fun u ->
                ch.z2 mat.size1 mat.size2 <| fun vt ->
                    La.svd mat (u,s,vt)
                    !"0に近いほど正確な解"
                    print.c "solve_homogeneq"
                    print.c s[mat.size1]
                    print.br
                    iter.range _1 mat.size1 <| fun i ->
                        f[i] <== asm.conj(vt[mat.size1,i])