...
build, then compiles or executes each program with its real runtime. The core
Python program runs in an environment with NumPy but without SciPy; a separate
program exercises the conditional SciPy import. To run the same check locally,
install Bash, GCC, GNU Fortran, Python with NumPy and SciPy, Node.js, and PHP
CLI, then run:
