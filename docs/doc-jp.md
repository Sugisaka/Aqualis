...
- 初回実行時にAqualisパッケージを取得できるよう、NuGet.orgに接続できる環境を用意する。以下のコード例では`#r "nuget: Aqualis, 188.0.3"`を使用する。
...
- 7行目：ソースファイルの出力先フォルダ。
- 9行目：NuGetからAqualis 188.0.3を読み込む。
...
以下の行を記述すると、以降の処理がデバッグモードになる。デバッグモードでは、配列のインデックス範囲や配列サイズなどを実行時にチェックするコードが生成される。

```fsharp
ctx.Setting.DebugMode ON
```

デバッグモードから通常モードに戻る際は以下のように記述する。
...
と書くと、整数値を文字列へ変換する際のフィールド幅が8文字になる。初期値は12文字である。この設定は画面出力、テキストファイル出力、整数値を含むファイル名などに使用される。

```fsharp
ctx.Setting.DoubleToStringFormat (27,17)
```

と書くと、倍精度浮動小数点値を文字列へ変換する際のフィールド幅が27文字、小数部の桁数が17桁になる。初期値も`(27,17)`である。この設定は画面出力やテキストファイル入出力などに使用される。

### コンパイルオプション

生成されたソースファイルのコンパイル時に使用するオプションを追加できる。以下は最適化オプション`-O2`を追加する例である。引数には先頭のハイフンを含めない。
