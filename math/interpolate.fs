...
namespace Aqualis
    ...
    module interpolate = 
            ...
            member _.y (x:double0) code =
                ch.i <| fun flag ->
                    ...
                    br.if1 (x.=X.[X.size1]) <| fun () ->
                        flag<==1
                        code(Y.[X.size1])
                    br.if1(flag.=0) <| fun () -> print.tt <| x++"is out of range:"++X.[1]++X.[X.size1]
        ...
        type splineInterpolateComplex(iscpx:bool) =
            ...
            member __.load(filename:string) =
                if iscpx then
                    io.fileInput (filename+"_x.dat") <| fun rd ->
                        ch.id <| fun (n,t) ->
                            rd.t n
                            x.allocate n
                            iter.num x.size1 <| fun i ->
                                rd.t t
                                x.[i] <== t
                    io.fileInput (filename+"_y.dat") <| fun rd ->
                        ch.idd <| fun (n,s,t) ->
                            rd.t n
                            y.allocate n
                            iter.num y.size1 <| fun i ->
                                rd.tt <| s++t
                                y.[i] <== s+asm.uj*t
                    io.fileInput (filename+"_g.dat") <| fun rd ->
                        ch.idd <| fun (n,s,t) ->
                            rd.t n
                            g.allocate n
                            iter.num g.size1 <| fun i ->
