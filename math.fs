...
namespace Aqualis
    ...
    module math = 
        ...
        let hn (hn:complex0) (n:int0,x:float0) =
        ...
        let hnarray (hn:int0->complex0) (N:int0) (n:int0,x:int0->float0) =
            // hn(x) = j * (-1)^n * x^n * [x^(-1) * (d/dx)]^n * [e^(-jx) * (x^(-1))]
            //c1(m): e^(-jx)(x^(-m))の係数
            ch.z1 (2*n+1) <| fun c1 ->
            ch.z1 (2*n+1) <| fun c2 ->
        ...
        //Legendre多項式 P_l
        let plgndr (pl:float0) (l:int0,x:float0) =
        ...
        let plgndrarray (pl:float1) (l:int0,x:float0) =
        ...
        let aplgndr (pl:float0) (l:int0, m:int0, x:float0) =
            ch.dd <| fun (fact,pll) ->
            ch.ddd <| fun (pmm,pmmp1,somx2) ->
                br.if1 (OR [m <. 0; m >. l; asm.abs(x) >. 1.0]) <| fun () ->
                    print.cn "Bad arguments in routine plgndr"
                pmm <== 1.0
                br.if1 (m >. 0) <| fun () ->
