    ...
    module Aqualis_main =
        ...
        let func (projectname:string) (code:unit->unit) =
            let fdeclare (typ:Etype,vtp:VarType,name:string) =
                ...
                |HTML ->
                    match vtp with 
                    |A0 -> typ.tostring pr.language + " :: " + name
                    |A1 0 -> typ.tostring pr.language + ",allocatable" + " :: " + name + "(:)"
                    |A2(0,0) -> typ.tostring pr.language + ",allocatable" + " :: " + name + "(:,:)"
                    |A3(0,0,0) -> typ.tostring pr.language + ",allocatable" + " :: " + name + "(:,:,:)"
                    |A1 _ -> typ.tostring pr.language + " :: " + name + "(:)"
                    |A2(_,_) -> typ.tostring pr.language + " :: " + name + "(:,:)"
                    |A3(_,_,_) -> typ.tostring pr.language + " :: " + name + "(:,:,:)"
                |_ ->
                    match vtp with 
                    |A0 -> typ.tostring pr.language + " :: " + name
                    |A1 0 -> typ.tostring pr.language + ",allocatable" + " :: " + name + "(:)"
            ...
            |Fortran ->
                ...
                pr.pwriter.close()
                //ソースファイル(関数部分)出力
                let writer = codeWriter(dir + "\\" + projectname + ".bee", 2, pr.language)
                writer.codewrite "!=============================================================================================\n"
                writer.codewrite("! Subroutine name: " + projectname + "\n")
                for _,(_,_,nm) in pr.arg.list do
                    writer.codewrite("!  " + nm + "\n")
                writer.codewrite "!============================================================================================="
                let argvar = String.Join(", ", pr.arg.list |> List.map(fun (_,(_,_,n)) -> n))
                ...
                //サブルーチン引数の定義
                for _,s in pr.arg.list do
                    writer.codewrite(fdeclare s)
                //グローバル変数の定義
                writer.codewrite(File.ReadAllText(dir + "\\" + projectname + "_var" + ".bee"))
                //メインコード
                writer.codewrite(File.ReadAllText(dir + "\\" + projectname + "_code.bee"))
                writer.indent.dec()
                ...
                pr.pwriter.delete()
                //呼び出しコードを記述
                let args = String.Join(", ", pr.arg.list |> List.map(fun (n,(_,_,_)) -> n))
                //もとの関数に戻る
                backProgram()
                pr.cwriter.codewrite("call" + " " + projectname + "(" + args + ")\n")
            |C99 ->
                pr.flist.add projectname
                nextProgram(dir,projectname,C99)
                code()
                declareall()
                pr.cwriter.close()
                pr.hwriter.close()
                ...
                let argvar =
                    ...
                    |> List.map (fun (_,(typ,vtp,n)) ->
                        match vtp with
                        |A1 _|A2 _|A3 _ -> typ.tostring pr.language + " *" + n
                        |_ -> typ.tostring pr.language + " *" + n)
                    |> fun s -> String.Join(", ", s)
                ...
                //もとの関数に戻る
                backProgram()
                pr.cwriter.codewrite(projectname + "(" + args + ");\n")
            |LaTeX ->
                pr.flist.add projectname
                nextProgram(dir,projectname,LaTeX)
                code()
                declareall()
                pr.cwriter.close()
                pr.hwriter.close()
            ...
            |HTML ->
                pr.flist.add projectname
                nextProgram(dir,projectname,HTML)
                code()
                declareall()
                pr.cwriter.close()
                pr.hwriter.close()
                ...
                //もとの関数に戻る
                backProgram()
                pr.cwriter.codewrite("\\(" + projectname + "(" + args + ")\\)<br/>\n")
            |Python ->
                pr.flist.add projectname
                nextProgram(dir,projectname,Python)
                code()
                declareall()
                pr.cwriter.close()
                pr.hwriter.close()
                ...
                File.Delete(dir + "\\" + projectname + "_str.bee")
                File.Delete(dir + "\\" + projectname + "_var.bee")
                File.Delete(dir + "\\" + projectname + "_par.bee")
                //もとの関数に戻る
                backProgram()
                pr.cwriter.codewrite(re_args + " = " + projectname + "(" + args + ")\n")
            |_ -> ()
        ...
        let Compile langgList dir projectname (aqver:string,codever:string) code =
            for lang in langgList do
                ...
                |Fortran -> 
                    ...
                    List.iter (fun (s:string) -> writer.codewrite("include " + s + "\n")) <| pr.hlist.list
                    //構造体の定義
                    writer.codewrite(File.ReadAllText(dir + "\\" + projectname + "_str" + ".bee"))
                    //グローバル変数の定義
                    writer.codewrite(File.ReadAllText(dir + "\\" + projectname + "_var" + ".bee"))
                    //メインコード
                    writer.codewrite(File.ReadAllText(dir + "\\" + projectname + "_code.bee"))
                    ...
                    writer.codewrite "\n"
                    for funname in pr.flist.list do
                    ...
                    if isOaccUsed then
                        wr.Write "#!/bin/bash\n"
                        wr.Write("\n")
                        let source = String.Join(" ", pr.slist.list)
                        let option = String.Join(" ", pr.olist.list)
                        ...
                        wr.Write "\n"
                        let source = String.Join(" ", pr.slist.list)
                        let option = "-ffree-line-length-none " + String.Join(" ", pr.olist.list)
