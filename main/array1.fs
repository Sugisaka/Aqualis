        ...
        member this.allocate(n1:num0) =
                ...
                |Var1(size1,name) ->
                    ...
                    |Fortran ->
                        ...
                        |A1 0 ->
                            this.size1 <== n1
                            pr.cwriter.codewrite("allocate("+name+"(1:"+this.size1.Expr.eval pr+")"+")"+"\n")
                        |_ -> 
                    ...
                    |C99 ->
                        ...
                        |A1 0 ->
                            this.size1 <== n1
                            pr.cwriter.codewrite(name+" = "+"("+typ.tostring pr.language+" *)"+"malloc("+"sizeof("+typ.tostring pr.language+")*"+this.size1.Expr.eval pr+");\n")
                        |_ -> 
                            pr.cwriter.codewrite("(Error:055-001 「"+name+"」は可変長1次元配列ではありません")
                    |LaTeX ->
                        match size1,typ with
                        |A1 0,It _ ->
                            pr.cwriter.codewrite("$"+name+" \\in \\mathbb{Z}^{"+n1.Expr.eval pr+"}$\\\\\n")
                        |A1 0,Dt   ->
                            pr.cwriter.codewrite("$"+name+" \\in \\mathbb{R}^{"+n1.Expr.eval pr+"}$\\\\\n")
                        |A1 0,Zt   ->
                            pr.cwriter.codewrite("$"+name+" \\in \\mathbb{C}^{"+n1.Expr.eval pr+"}$\\\\\n")
                        |_ -> 
                            pr.cwriter.codewrite("(Error:055-001 「"+name+"」は可変長1次元配列ではありません")
                    |HTML ->
                        match size1,typ with
                        |A1 0,It _ ->
                            pr.cwriter.codewrite("\\("+name+" \\in \\mathbb{Z}^{"+n1.Expr.eval pr+"}\\)<br>\n")
                        |A1 0,Dt   ->
                            pr.cwriter.codewrite("\\("+name+" \\in \\mathbb{R}^{"+n1.Expr.eval pr+"}\\)<br>\n")
                        |A1 0,Zt   ->
                            pr.cwriter.codewrite("\\("+name+" \\in \\mathbb{C}^{"+n1.Expr.eval pr+"}\\)<br>\n")
                        |_ -> 
                            pr.cwriter.codewrite("(Error:055-001 「"+name+"」は可変長1次元配列ではありません")
                    |Python ->
                        match size1 with
                        |A1 0 ->
                            this.size1 <== n1
                            match typ with
                            |Structure sname -> pr.cwriter.codewrite(name+" = numpy.array(["+sname+"() for _ in range(int("+this.size1.Expr.eval pr+"))], dtype=object)\n")
                            |It _ |It 1      -> pr.cwriter.codewrite(name+" = numpy.zeros("+this.size1.Expr.eval pr+", dtype=int)\n")
                            |Zt              -> pr.cwriter.codewrite(name+" = numpy.zeros("+this.size1.Expr.eval pr+", dtype=numpy.complex128)\n")
