...
type optimization() =
    ...
    static member findmin (m:int) (x0_:float1,df:float1) (dd:float0) (f:float0->float1->unit) (xx:float1) =
        ...
        ch.i <| fun counter ->
            ...
            iter.num df.size1 <| fun i ->
                norm_df.num0 <== norm_df.num0 + df[i] * df[i]
            norm_df <== asm.sqrt(norm_df)
            xa.foreach <| fun i -> xa[i].num0 <== x0_[i]
            f fa xa
            xb.foreach <| fun i -> xb[i].num0 <== x0_[i] + dd * df[i] / norm_df.num0
            f fb xb
    ...
    static member findmin_GradientDescent (n:int,m:int) (x0:float1) (dd0:num0) (f:float0->float1->unit) (df:float1->float1->unit) (stepProc:((int0*float1)->unit)option) =
        match stepProc with
          |Some(pu) ->
                pu (_0,x0)
          |None ->
    ...
    static member findmin_ConjugateGradient1 (n:int,m:int) (x0:float1) (dd0:float0) (f:float0->float1->unit) (df:float1->float1->unit) (fH:float2->float1->unit) (stepProc:((int0*float1)->unit)option) =
        match stepProc with
          |Some(pu) ->
                pu (_0,x0)
          |None ->
    ...
    /// <param name="f">目的関数</param>
    /// <param name="df">目的関数の勾配</param>
    static member findmin_ConjugateGradient2 (n:int,m:int) (x0:float1) (dd0:num0) (f:float0->float1->unit) (df:float1->float1->unit) (stepProc:((int0*float1)->unit)option) =
        match stepProc with
          |Some(pu) ->
                pu (_0,x0)
          |None ->
    ...
    /// <param name="df"></param>
    /// <param name="fH"></param>
    static member findmin_Newton (n:int,m:int) (x0:float1) (dd0:float0) (f:float0->float1->unit) (df:float1->float1->unit) (fH:float2->float1->unit) (stepProc:((int0*float1)->unit)option) =
        match stepProc with
          |Some(pu) ->
                pu (_0,x0)
          |None ->
    ...
    /// <param name="f"></param>
    /// <param name="df"></param>
    static member findmin_quasiNewton (n:int,m:int) (x0:float1) (dd0:float0) (f:float0->float1->unit) (df:float1->float1->unit) (stepProc:((int0*float1)->unit)option) =
        match stepProc with
          |Some(pu) ->
                pu (_0,x0)
          |None ->
                ...
                ch.d1 x0.size1 <| fun df0 ->
                    ...
                    La.matmul (B,df0) <| fun a ->
                        ...
                        ch.d1 x0.size1 <| fun xx ->
                            ...
                            s.foreach <| fun j -> s[j].num0 <== xx[j].num0 - x0[j]
                            df1 <== df0
                            x0.foreach <| fun i -> x0[i] <== xx[i].num0
                            match stepProc with
                              |Some(pu) ->
                                    pu(i,x0)
                              |None ->
