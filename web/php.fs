...
module num0ForPHP =
    type num0 with
        ...
        static member array(arrayname:string,data:list<string*num0>) = 
            let c = num0.var arrayname
            codewrite ("<?php "+arrayname+" = array(); ?>")
            codewrite ("<?php "+arrayname+"[] = array("+String.Join(",",data |> List.map (fun (a,b) -> "'"+a+"'=>"+b.Expr.eval (programList[prIndex])))+"); ?>")
            c
        ...
        member this.push (x:list<exprString>) = 
            codewrite (
                ...
                "); ?>")
        member this.push (x:list<num0>) = codewrite ("<?php array_push(" + this.Expr.eval (programList[prIndex]) + ", " + String.Join(",",List.map(fun (q:num0) -> q.Expr.eval (programList[prIndex])) x) + "); ?>")
        ///配列に文字列要素を複数追加
...
type PHPdata(x:string) =
    ...
    member this.foreach code =
        ch.i <| fun i ->
            php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"=0; "+i.Expr.eval (programList[prIndex])+"<count("+this.code+"); "+i.Expr.eval (programList[prIndex])+"++):")
    ...
    member this.foreach (key:num0,value:num0) = fun code ->
        ch.i <| fun i ->
            php.phpcode <| fun () -> codewrite("foreach("+this.code+" as "+key.Expr.eval (programList[prIndex])+" => "+value.Expr.eval (programList[prIndex])+"):")
            code()
...
and php =
    ...
    /// ファイルにテキストを書き込み
    static member file_put_contents (filename:num0,x:num0) = php.phpcode <| fun () -> codewrite("file_put_contents("+filename.Expr.eval (programList[prIndex])+","+x.Expr.eval (programList[prIndex])+");")
    /// ファイルにテキストを書き込み
    static member file_put_contents (filename:string,x:num0) = php.phpcode <| fun () -> codewrite("file_put_contents("+"\""+filename+"\""+","+x.Expr.eval (programList[prIndex])+");")
    /// JSONファイルをデコード
    static member json_decode (x:num0,p:bool) = num0(Var(Nt, "json_decode("+x.Expr.eval (programList[prIndex])+","+p.ToString()+");",NaN))
    /// JSONファイルをエンコード
    ...
    /// 配列に要素を追加
    static member array_push(a:num0,el:num0) = php.phpcode <| fun () -> codewrite("array_push("+a.Expr.eval (programList[prIndex])+","+el.Expr.eval (programList[prIndex])+")")
    /// ファイル内のテキストを配列に格納
    ...
    /// ファイルに書き込み
    static member fwrite(fp:num0,t:exprString) = php.phpcode <| fun () -> codewrite("fwrite("+fp.Expr.eval (programList[prIndex])+", "+t.toString("",Direct)+");")
    /// ファイルに書き込み
    static member fwrite(fp:num0,t:string) = php.fwrite(fp,Str t)
    /// ファイルに書き込み
    static member fwrite(fp:num0,t:int) = php.phpcode <| fun () -> codewrite("fwrite("+fp.Expr.eval (programList[prIndex])+", "+t.ToString()+");")
    /// ファイルにShift-JISで書き込み
    static member fwrite_SJIS(fp:num0,t:num0) = php.phpcode <| fun () -> codewrite("fwrite("+fp.Expr.eval (programList[prIndex])+", mb_convert_encoding("+t.Expr.eval (programList[prIndex])+", 'SJIS-win', 'UTF-8'));")
    /// ファイルにShift-JISで書き込み
    static member fwrite_SJIS(fp:num0,t:string) = php.phpcode <| fun () -> codewrite("fwrite("+fp.Expr.eval (programList[prIndex])+", mb_convert_encoding(\""+t+"\", 'SJIS-win', 'UTF-8'));")
    /// ファイルを閉じる
    static member fclose(filename:num0) = php.phpcode <| fun () -> codewrite("fclose("+filename.Expr.eval (programList[prIndex])+");")
    /// 正規表現
    static member preg_match(p:string,text:num0,mat:num0) = php.phpcode <| fun () -> codewrite("preg_match('"+p+"',"+text.Expr.eval (programList[prIndex])+","+mat.Expr.eval (programList[prIndex])+");")
    /// ファイルのダウンロード
    ...
    /// HTTPヘッダを取得
    static member header(data:num0) = php.phpcode <| fun () -> codewrite("header("+data.Expr.eval (programList[prIndex])+")")
    /// HTTPヘッダを取得
    ...
    /// 配列のソート
    static member sort(data:num0) = php.phpcode <| fun () -> codewrite("sort("+data.Expr.eval (programList[prIndex])+")")
    /// 整数に変換
    ...
    /// ファイル削除
    static member unlink(data:num0) = php.phpcode <| fun () -> codewrite("unlink("+data.Expr.eval (programList[prIndex])+")")
    /// 配列要素をランダムに入れ替え
    static member shuffle(data:num0) = php.phpcode <| fun () -> codewrite("shuffle("+data.Expr.eval (programList[prIndex])+")")
    /// タイムゾーン設定
    ...
    static member sendMail(body:string,subject:string,smtp:string,fromAddress:string,toAddress:string) =
        ...
        php.phpcode <| fun () -> codewrite("exec("+cmd.Expr.eval (programList[prIndex])+");")
    /// メール送信
    static member sendMail(body:num0,subject:num0,fromAddress:string,toAddress:num0) =
        ...
        php.phpcode <| fun () -> codewrite("mb_send_mail("+toAddress.Expr.eval (programList[prIndex])+","+subject.Expr.eval (programList[prIndex])+","+body.Expr.eval (programList[prIndex])+","+"\"From: "+fromAddress+"\");")
    /// メール送信
    ...
    static member file_download(file:num0) =
        ...
        php.phpcode <| fun () ->
            codewrite("header('Content-disposition: attachment; filename='.basename("+file.Expr.eval (programList[prIndex])+"));")
        php.phpcode <| fun () ->
            codewrite("header('Content-Length: '.filesize("+file.Expr.eval (programList[prIndex])+"));")
        php.phpcode <| fun () ->
            codewrite("while (ob_get_level()) { ob_end_clean(); }")
        php.phpcode <| fun () ->
            codewrite("readfile("+file.Expr.eval (programList[prIndex])+");")
        php.phpcode <| fun () ->
            codewrite("exit;")
    /// ファイルパスからファイル名取得
...
//         php.phpcode <| fun () -> codewrite("if("+s.Expr.eval (programList[prIndex])+"):")
//         code()
//     member this.ElseIf(s:bool0) = fun code ->
//         php.phpcode <| fun () -> codewrite("elseif("+s.Expr.eval (programList[prIndex])+"):")
//         code()
//     member this.Else code =
//         php.phpcode <| fun () -> codewrite "else:"
//         code()
// type br =
//     static member branch code =
//         let b = bh()
//         code b
//         php.phpcode <| fun () -> codewrite "endif;"
...
//        php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"="+i1.Expr.eval (programList[prIndex])+"; "+i.Expr.eval (programList[prIndex])+"<="+i2.Expr.eval (programList[prIndex])+"; "+i.Expr.eval (programList[prIndex])+"++):")
//        code(i)
//        php.phpcode <| fun () -> codewrite("endfor;")
//    static member range (i1:num0,i2:int) = fun code ->
//        icounter <- icounter + 1
//        let i = num0.var("ic"+icounter.ToString())
//        php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"="+i1.Expr.eval (programList[prIndex])+"; "+i.Expr.eval (programList[prIndex])+"<="+i2.ToString()+"; "+i.Expr.eval (programList[prIndex])+"++):")
//        code(i)
//        php.phpcode <| fun () -> codewrite("endfor;")
//    static member range (i1:int,i2:num0) = fun code ->
//        icounter <- icounter + 1
//        let i = num0.var("ic"+icounter.ToString())
//        php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"="+i1.ToString()+"; "+i.Expr.eval (programList[prIndex])+"<="+i2.Expr.eval (programList[prIndex])+"; "+i.Expr.eval (programList[prIndex])+"++):")
//        code(i)
//        php.phpcode <| fun () -> codewrite("endfor;")
//    static member range (i1:int,i2:int) = fun code ->
//        icounter <- icounter + 1
//        let i = num0.var("ic"+icounter.ToString())
//        php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"="+i1.ToString()+"; "+i.Expr.eval (programList[prIndex])+"<="+i2.ToString()+"; "+i.Expr.eval (programList[prIndex])+"++):")
//        code(i)
//        php.phpcode <| fun () -> codewrite("endfor;")
//    static member foreach (array:num0) = fun code ->
//        icounter <- icounter + 1
//        let i = num0.var("ic"+icounter.ToString())
//        php.phpcode <| fun () -> codewrite("for("+i.Expr.eval (programList[prIndex])+"=0; "+i.Expr.eval (programList[prIndex])+"<count("+array.Expr.eval (programList[prIndex])+"); "+i.Expr.eval (programList[prIndex])+"++):")
//        code(i)
//        php.phpcode <| fun () -> codewrite("endfor;")
//    static member foreach (array:num0,key:num0,value:num0) = fun code ->
//        icounter <- icounter + 1
//        let i = num0.var("ic"+icounter.ToString())
//        php.phpcode <| fun () -> codewrite("foreach("+array.Expr.eval (programList[prIndex])+" as "+key.Expr.eval (programList[prIndex])+" => "+value.Expr.eval (programList[prIndex])+"):")
//        code()
//        php.phpcode <| fun () -> codewrite("endforeach;")