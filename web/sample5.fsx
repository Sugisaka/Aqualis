...
module memberinfo =
    ...
    /// メンバー情報（PHP用）
    type _Member(var:PHPvar) =
        member _.ID = var["ID"]
        member _.PassWord = var["PassWord"]
        
    /// メンバー配列（PHP用）
    type _Members(var:PHPvar) =
        member _.Item with get(i:int) = _Member(var[i])
        member _.Item with get(i:PHPvar) = _Member(var[i])
        
    /// メンバーデータ（PHP用）
    type MemberData(varname:string) =
        let memberData = PHPvar.var varname
        ...
        html.h1 "ログインページサンプル" <| fun () ->
            ...
            html.form "main.php" <| fun () ->
                ...
                <| fun () ->
                    ...
                    let loginState = PHPvar.var("loginState",0)
                    // ログイン情報検証
