...
type FileLoginThrottle(context:Aqualis, options:FileLoginThrottleOptions) =
    ...
    member _.EmitDefinitions() =
        ...
        let source = [
            ...
            "$normalizedEntries = [];"
            "foreach ($state['entries'] as $key => $entry) { if (!is_string($key) || preg_match('/\\A(?:ip|user|user_ip):[0-9a-f]{64}\\z/D', $key) !== 1 || !is_array($entry) || !isset($entry['count'], $entry['window_start']) || !is_int($entry['count']) || $entry['count'] < 0 || !is_int($entry['window_start']) || $entry['window_start'] < 0) { return null; } $entryLimit = strncmp($key, 'ip:', 3) === 0 ? " + maxIp + " : " + maxUser + "; if ($entry['count'] > $entryLimit) { return null; } $normalizedEntries[$key] = ['count' => $entry['count'], 'window_start' => $entry['window_start']]; }"
