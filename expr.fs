    ...
    type num0 =
        ...
        static member (+) (x:num0,y:num0) : num0 =
            if isEqSimplify then
              match x,y with
                (* x+0 *)
                |_,Int_e 0|_,Dbl_e 0.0 -> x
                (* 0+y *)
                |Int_e 0,_|Dbl_e 0.0,_ -> y
                (* (-x)+(-y) *)
                ...
                |Inv(_,v1),_ -> y-v1
                (* x+[整数定数] *)
                |_,Int_e v2 when v2<0   -> x-Int_e(-v2)
                (* x+[小数定数] *)
                |_,Dbl_e v2 when v2<0.0 -> x-Dbl_e(-v2)
                (* [整数定数]+[整数定数] *)
                |Int_e v1,Int_e v2 -> Int_e(v1+v2)
                (* [整数定数]+[小数定数] *)
                |Int_e v1,Dbl_e v2 -> Dbl_e((double v1)+v2)
                (* [小数定数]+[整数定数] *)
                |Dbl_e v1,Int_e v2 -> Dbl_e(v1+(double v2))
                (* [小数定数]+[小数定数] *)
                |Dbl_e v1,Dbl_e v2 -> Dbl_e(v1+v2)
                (* (x1+x2+…)+(y1+y2+…) *)
                |Add(_,v1),Add(_,v2) -> Add(num0.ptype(x.etype,y.etype),v1@v2)
                (* (x1-x2)+(y1+y2+…) *)
                |Sub(_,u1,u2),Add(t,v2) -> Add(t,[u1;-u2;]@v2)
                (* (x1+x2+…)+(y1-y2) *)
                |Add(t,v1),Sub(_,u1,u2) -> Add(t,v1@[u1;-u2])
                (* x+(y1+y2+…) *)
                |_,Add(t,v2) -> Add(t,x::v2)
                (* (x1+x2+…)+y *)
                |Add(t,v1),_ -> Add(t,v1@[y])
                (* (x1-x2)+(y1-y2) *)
                |Sub(t1,v1,v2),Sub(t2,u1,u2) -> Add(num0.ptype(t1,t2),[v1;-v2;u1;-u2])
                (* x+(y1+y2+…) *)
                |_,Sub(_,v1,v2) -> Add(num0.ptype(x.etype,y.etype),[x;v1;-v2])
                (* (x1-x2)+y *)
                |Sub(_,v1,v2),_ -> Add(num0.ptype(x.etype,y.etype),[v1;-v2;y])
                (* x+y *)
        ...
        static member (-) (x:num0,y:num0) : num0 =
            if isEqSimplify then
              match x,y with
                ...
                |_,Int_e 0|_,Dbl_e 0.0 -> x
                (* 0-y *)
                |Int_e 0,_|Dbl_e 0.0,_ -> -y //Inv(y.etype,y)
                (* (-x)-(-y) *)
                |Inv(_,v1),Inv(_,v2) -> v2-v1
                (* x-(-y) *)
                |_,Inv(_,v2) -> x+v2
                (* (-x)-y *)
                |Inv(_,v1),_ -> -(y+v1)
                ...
                |Dbl_e v1,Dbl_e v2 -> Dbl_e(v1-v2)
                (* (x1+x2+…)-(y1+y2+…) *)
                |Add(_,v1),Add(_,v2) -> Add(num0.ptype(x.etype,y.etype),v1@(v2|>List.map(fun v -> -v)))
                (* (x1-x2)-(y1+y2+…) *)
                |Sub(_,u1,u2),Add(t,v2) -> Add(t,[u1;-u2;]@(v2|>List.map(fun v -> -v)))
                (* (x1+x2+…)-(y1-y2) *)
                |Add(t,v1),Sub(_,u1,u2) -> Add(t,v1@[-u1;u2])
                (* x-(y1+y2+…) *)
                |_,Add(t,v2) -> Add(t,x::(v2|>List.map(fun v -> -v)))
                (* (x1+x2+…)-y *)
                |Add(t,v1),_ -> Add(t,v1@[-y])
                (* (x1-x2)-(y1-y2) *)
                |Sub(t1,v1,v2),Sub(t2,u1,u2) -> Add(num0.ptype(t1,t2),[v1;-v2;-u1;u2])
                (* x-(y1-y2) *)
                |_,Sub(_,v1,v2) -> Add(num0.ptype(x,y),[x;-v1;v2])
                (* (x1-x2)-y *)
                |Sub(_,v1,v2),_ -> Add(num0.ptype(x,y),[v1;-v2;-y])
                (* x-y *)
        ...
        static member (*) (x:num0,y:num0) : num0 =
            if isEqSimplify then
              match x,y with
                (* x*0 *)
                |Int_e 0,_|Dbl_e 0.0,_|_,Int_e 0|_,Dbl_e 0.0 -> Int_e 0
                (* 1*y *)
                |Int_e 1,_|Dbl_e 1.0,_ -> y
                (* x*1 *)
                |_,Int_e 1|_,Dbl_e 1.0 -> x
                (* [負の整数定数]*y *)
                |Int_e v1,_ when v1<0   -> -((-v1)*y)
                (* [負の小数定数]*y *)
                |Dbl_e v1,_ when v1<0.0 -> -((-v1)*y)
                (* x*[負の整数定数] *)
                |_,Int_e v2 when v2<0   -> -(x*(-v2))
                (* x*[負の小数定数] *)
                |_,Dbl_e v2 when v2<0.0 -> -(x*(-v2))
                (* x*(-y) *)
                |_,Inv(_,v2) -> -(x*v2)
                (* (-x)*y *)
                |Inv(_,v1),_ -> -(v1*y)
            ...
            if isEqSimplify then
              match x.etype,y.etype with
                ...
                |_ ->
                  match x,y with
                    (* x/0 *)
                    |_,Int_e 0 |_,Dbl_e 0.0 ->
                      Console.WriteLine("Error: ゼロ割りを検出しました")
                      Console.Read() |> ignore
                      NaN
                    (* 0/y *)
                    |Int_e 0,_ -> Int_e 0
                    |Dbl_e 0.0,_ -> Dbl_e 0.0
                    |_,Dbl_e 1.0 -> x
                    (* [負の整数定数]/[負の整数定数] *)
                    |Dbl_e v1,Int_e v2 when v1<0.0 && v2<0 ->  Dbl_e(( v1)/(double -v2))
                    (* [負の整数定数]/[整数定数] *)
                    |Dbl_e v1,Int_e v2 when v1<0.0         -> -Dbl_e((-v1)/(double v2))
                    (* [整数定数]/[負の整数定数] *)
                    |Dbl_e v1,Int_e v2 when           v2<0 -> -Dbl_e(( v1)/(-(double v2)))
                    (* [整数定数]/[整数定数] *)
                    |Dbl_e v1,Int_e v2                     ->  Dbl_e(v1/(double v2))
                    (* [負の整数定数]/[負の小数定数] *)
                    |Int_e v1,Dbl_e v2 when v1<0 && v2<0.0   ->  Dbl_e((double  v1)/(-v2))
                    (* [負の整数定数]/[小数定数] *)
                    |Int_e v1,Dbl_e v2 when v1<0             -> -Dbl_e((double -v1)/( v2))
                    (* [整数定数]/[負の小数定数] *)
                    |Int_e v1,Dbl_e v2 when         v2<0.0   -> -Dbl_e((double  v1)/(-v2))
                    (* [整数定数]/[小数定数] *)
                    |Int_e v1,Dbl_e v2                       ->  Dbl_e((double v1)/v2)
                    (* [負の小数定数]/[負の小数定数] *)
                    |Dbl_e v1,Dbl_e v2 when v1<0.0 && v2<0.0 ->  Dbl_e((-v1)/(-v2))
                    (* [負の小数定数]/[小数定数] *)
                    |Dbl_e v1,Dbl_e v2 when v1<0.0           -> -Dbl_e((-v1)/( v2))
                    (* [小数定数]/[負の小数定数] *)
                    |Dbl_e v1,Dbl_e v2 when v2<0.0           -> -Dbl_e(( v1)/(-v2))
                    (* [小数定数]/[小数定数] *)
                    |Dbl_e v1,Dbl_e v2                       ->  Dbl_e(v1/v2)
                    (* (-x)/[小数定数] *)
                    |Dbl_e v1,_ when v1<0.0 -> -((-v1)/y)
              ...
              match x.etype,y.etype with
                |It _,It _ ->
                  match x,y with
                    ...
                    |_,Int_e v2 when v2<0   -> (x%(-v2))
                    |Int_e v1,Int_e v2 -> Int_e(v1%v2)
                    |_,(Add(t,_)|Sub(t,_,_)|Mul(t,_,_)|Div(t,_,_)) -> Mod(x,Par(t,"(",")",y))
                    |(Add(t,_)|Sub(t,_,_)),_ -> Mod(Par(t,"(",")",x),y)
                    |_ -> Mod(x,y)
                |_ ->
                  NaN
            else
